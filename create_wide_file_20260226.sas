...
libname b "C:\Users\rmjlja9\Dropbox (UCL)\hiv synthesis ssa unified program\output files\zimbabwe\mihpsa_p2_all_20250324_out";		* all options in one output;


data base_26_02_2026; set b.out: ;
	...
	* Overwrite s_sw_program_visit with 0s in all options except SQ (0) and FSW program (8) [error in options code not setting all sw_program_visit=0 at year_interv] JAS Feb26;
	if cald >= 2023 and option ne 1 and option ne 8 then s_sw_program_visit = 0;
...
/*proc contents data=a.base_26_02_2026;run;							* show the contents of the input SAS file; */
proc freq data=base_26_02_2026; table run cald option; where cald=2030; run;
* 137 runs;
* Extra test options are 32 (mens clinics), 33 (FSW prog), 38-40 (ART support);


proc sort data=base_26_02_2026; 
...
if s_dcost_cascade_interventions=. then s_dcost_cascade_interventions=0;	*Delete this one?;
...
dcost = dart_cost_y + dadc_cost + dcd4_cost + dvl_cost + dvis_cost + dnon_tb_who3_cost + dcot_cost + dtb_cost + dres_cost + 
		dtest_cost + d_t_adh_int_cost + dswitchline_cost + dcost_drug_level_test+dcost_cascade_interventions + dcost_circ + dcost_condom_dn + 
...
* n_sw_program_visit;			n_sw_program_visit = s_sw_program_visit * sf;		* Removed x4 because we are using this as a proxy for total number of SW in a program in one year (not costing by number of visits) JASFeb2026;
* n_sw_inprog_ly;				n_sw_inprog_ly = s_sw_inprog_ly * sf;
* n_sw_inprog_ever;				*n_sw_inprog_ever = s_sw_inprog_ever *sf;
* n_diag_sw_inprog;				n_diag_sw_inprog = s_diag_sw_inprog * sf;	*JASJan2025;
...
* n_tested_as_sw;				n_tested_as_sw = s_tested_as_sw * sf * 4;		*JASNov2024;
* n_tested_swprog;				n_tested_swprog = s_tested_f_progsw * sf * 4;
...
* n_tested_symptoms_not_hiv;	n_tested_symptoms_not_hiv = s_tested_symptoms_not_hiv * sf * 4;*JSJan2025;
...
/*
Ntests_FACNOSYMPT_M1599_M: Number of tests for HIV at facility, excluding ANC & PD, contacts testing for HIV at the facility and testing of FSW, among males15+ years old without symptoms
Ntests_FACNOSYMPT_F1599_M: Number of tests for HIV at facility, excluding ANC & PD, contacts testing for HIV at the facility and testing of FSW, among femle 15+ years old without symptoms
Ntests_IndexFAC_M1599_M: Tests conducted among males contacts 15+ years old tested for HIV at the facility
Ntests_IndexFAC_F1599_M: Tests conducted among females contacts 15+ years old tested for HIV at the facility
*/

/* JANUARY 2025:
*Types of testing in model (mutually exclusive);
tested_tb
tested_circ -> tested_m_circ
tested_symptoms_not_hiv
tested_anc
tested_labdel
tested_pd
tested_as_sw
tested_general
testfor_prep_any -> n_tested_startprep [testfor_prep_oral, testfor_prep_inj, testfor_prep_vr]
tested_onprep_any -> n_tested_onprep
tested_rsprep_any -> n_tested_rsprep
	together these are summed -> n_tested_prep
tested_sbcc_program -> n_tested_sbcc
tested_sympt -> n_tested_m_sympt + n_tested_w_sympt
tested_due_to_self_test -> n_tested_due_to_st
...
* test_proppos_sympt; 			if s_tested_m_sympt gt 0 and s_tested_f_sympt gt 0 then test_proppos_sympt = (s_diag_this_period_m_sympt + s_diag_this_period_f_sympt) /(s_tested_m_sympt + s_tested_f_sympt);*VCFeb2024;
...
* n_new_mcirc_1549m; 			n_new_mcirc_1549m = (s_new_mcirc_1519m+s_new_mcirc_2024m+s_new_mcirc_2529m+s_new_mcirc_3034m+s_new_mcirc_3539m+s_new_mcirc_4044m+ s_new_mcirc_4549m) * sf * 4;
...
* prop_1549_onprep;				prop_1549_onprep =   max(s_prep_any, 0) / ((s_alive1549_w + s_alive1549_m) - s_hiv1549)  ;
...
* p_diag_all_sw;				if s_sw_1564 > 0 then p_diag_all_sw = s_diag_sw / s_sw_1564; *VCFeb2023;
* p_diag_sw;					if s_hiv_sw > 0 then p_diag_sw = s_diag_sw / s_hiv_sw;	*JASJan2025;
...
* p_vg1000_all;					p_vg1000_all = s_vg1000 / s_alive ;
...
* n_total_yllag;				n_total_yllag = s_total_yllag * sf;		*YLLS undiscounted;
* n_dyll_GBD;					n_dyll_GBD = s_dyll_GBD * sf;			*YLLS discounted at 3%;
...
n_new_inf1564m = (s_primary1549m + s_primary5054m + s_primary5559m + s_primary6064m)* sf * 4;
n_new_inf1564w = (s_primary1549w + s_primary5054w + s_primary5559w + s_primary6064w)* sf * 4;
...
*Extra MIHPSA outputs;
*n_attend_mens_clinic;				n_attend_mens_clinic = s_attend_mens_clinic * sf; 	*JASNov24; 
*n_hiv_mens_clinic;					n_hiv_mens_clinic = s_attend_mens_clinic * sf; 		*JASNov24; 
*n_diag_mens_clinic;				n_diag_mens_clinic = s_diag_mens_clinic * sf; 		*JASNov24; 
*n_onart_mens_clinic;				n_onart_mens_clinic = s_onart_mens_clinic * sf;		*JASNov24; 

*n_access_adolescent_supp;			n_access_adolescent_supp = s_access_adolescent_supp * sf; 	*JASNov24; 
*n_hiv_adolescent_supp;				n_hiv_adolescent_supp = s_hiv_adolescent_supp * sf; 		*JASNov24; 
*s_diag_adolescent_supp;			n_diag_adolescent_supp = s_diag_adolescent_supp * sf; 		*JASNov24; 
*n_onart_adolescent_supp;			n_onart_adolescent_supp = s_onart_adolescent_supp * sf;		*JASNov24; 
*n_vls_adolescent_supp;				n_vls_adolescent_supp = s_vls_adolescent_supp * sf;			*JASNov24; 

*n_access_adult_adh_supp;			n_access_adult_adh_supp = s_access_adult_adh_supp * sf; 	*JASNov24; 
*n_hiv_adult_adh_supp;				n_hiv_adult_adh_supp = s_hiv_adult_adh_supp * sf; 			*JASNov24; 
*s_diag_adult_adh_supp;				n_diag_adult_adh_supp = s_diag_adult_adh_supp * sf; 		*JASNov24; 
*n_onart_adult_adh_supp;			n_onart_adult_adh_supp = s_onart_adult_adh_supp * sf;		*JASNov24; 
*n_vls_adult_adh_supp;				n_vls_adult_adh_supp = s_vls_adult_adh_supp * sf;			*JASNov24; 

*n_access_adult_ret_supp;			n_access_adult_ret_supp = s_access_adult_ret_supp * sf; 	*JASNov24; 
*n_hiv_adult_ret_supp;				n_hiv_adult_ret_supp = s_hiv_adult_ret_supp * sf; 			*JASNov24; 
*s_diag_adult_ret_supp;				n_diag_adult_ret_supp = s_diag_adult_ret_supp * sf; 		*JASNov24; 
*n_onart_adult_ret_supp;			n_onart_adult_ret_supp = s_onart_adult_ret_supp * sf;		*JASNov24; 
...
p_diag_m1524 p_diag_w1524 p_diag_all_sw p_diag_sw n_cm n_vm p_vm_ly_onart n_pcp_p p_onart_cd4_l200
...
p_onart_vl1000 p_vl1000 p_vg1000 p_vl1000_m  p_vl1000_w   p_vl1000_m_1524  p_vl1000_w_1524  p_vg1000_all  
...
n_sw_1564 n_sw_1549 prev_sti_sw n_sw_program_visit n_sw_inprog_ly  /*n_sw_inprog_ever*/ n_diag_sw_inprog
...
n_tested_tb n_tested_symptoms_not_hiv n_tested_general n_tested_startprep n_tested_onprep n_tested_rsprep n_tested_prep
...
n_prep_w_1549		n_prep_m_1549
n_prep_oral			n_prep_inj		n_prep_vr
n_prep_oral_w		n_prep_oral_m	
n_prep_inj_w		n_prep_inj_m
n_prep_vr_w
n_prep_w			n_prep_m
n_prep_1524w		n_prep_sw		n_prep_sdc		n_prep_plw
prop_1549_onprep	prop_1564_onprep
prop_w_1524_onprep  p_w1524newpge1_onprep 
...
n_agyw					n_fsw					n_sdc					n_plw
p_prep_agyw				p_prep_fsw				p_prep_sdc				p_prep_plw
prop_1564_hivneg_onprep prop_sw_onprep p_prep_adhg80
n_hiv1_prep			p_hiv1_prep
n_start_rest_prep_oral
n_elig_prep_w_1524	n_elig_prep_w_2534	n_elig_prep_w_3544
n_elig_prep_sw		n_elig_prep_sdc		n_elig_prep_plw
n_prep_w_1524		n_prep_w_2534		n_prep_w_3544
...
rate_choose_stop_prep_oral rate_choose_stop_prep_inj rate_choose_stop_prep_vr 
...
prep_any_strategy higher_future_prep_coverage  sens_test_prep
...
e_decr_hard_reach_year_i   prep_any_strategy 
...
eff_rate_choose_stop_prep_oral eff_rate_choose_stop_prep_inj eff_rate_choose_stop_prep_vr 																			   
sens_vct_test_type_3  prep_efficacy   p_ep
...
data a.l_base_26_02_2026; set y;
if cald=. then delete;run;
proc freq data=a.l_base_26_02_2026; table option; where cald=2030;run;
/*proc contents data=a.l_base_26_02_2026; run;*/



/*proc freq data=a.l_base_26_02_2026; table p_w1524newpge1_onprep n_w1524_newp_ge1;run;*/
/*proc freq data=a.l_base_26_02_2026;*/
...
/*proc freq data=a.l_base_26_02_2026; table incidence1549; where option=1;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_diag_progsw; where option=0;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_inprog_ly; where option=0;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_inprog_ly; where option=1;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_inprog_ly; where option=8;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_inprog_ly; where option=33;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_program_visit; where option=0;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_program_visit; where option=1;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_sw_program_visit; where option=8;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_attend_mens_clinic; where option=0;run;*/
/*proc freq data=a.l_base_26_02_2026; table n_access_adult_ret_supp; where option=30;run;*/
