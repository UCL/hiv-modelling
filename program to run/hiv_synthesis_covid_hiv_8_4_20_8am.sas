...
if 2020.5 <= caldate{t} < 2021 and swprog_disrup_covid = 1 then do;
	eff_sw_program = 0 ; 
	sw_test_6mthly = 0; 
	eff_sw_higher_int = sw_higher_int ; 
	eff_prob_sw_lower_adh = prob_sw_lower_adh ; 
...
;
		vmmc_disrup_covid = 0 ; if u_43 < 0.5 then vmmc_disrup_covid = 1 ;
		condom_disrup_covid = 0; if u_44 < 0.5 then condom_disrup_covid = 1 ;
		prep_disrup_covid = 0; if u_45 < 0.5 then prep_disrup_covid = 1 ;
		swprog_disrup_covid = 0; if u_46 < 0.5 then swprog_disrup_covid = 1 ;
		testing_disrup_covid = 0; if u_47 < 0.5 then testing_disrup_covid = 1 ;
		art_tld_disrup_covid = 0; if u_48 < 0.5 then art_tld_disrup_covid = 1 ;
		art_tld_eod_disrup_covid = 0; if u_49 < 0.5 then art_tld_eod_disrup_covid = 1 ;
		art_init_disrup_covid = 0; if u_50 < 0.5 then art_init_disrup_covid = 1 ;
		vl_adh_switch_disrup_covid = 0; if u_51 < 0.5 then vl_adh_switch_disrup_covid = 1 ;
		cotrim_disrup_covid = 0; if u_52 < 0.5 then cotrim_disrup_covid = 1 ;
		no_art_disrup_covid = 0; if u_53 < 0.5 then no_art_disrup_covid = 1 ;
...
if art_tld_disrup_covid = 1 or art_tld_eod_disrup_covid = 1 then reg_option = 104 ;
...
if 2020.5 <= caldate{t} < 2021 and vl_adh_switch_disrup_covid = 1 then do; eff_prob_vl_meas_done=0; eff_pr_switch_line=0; end; 
...
if 2020.5 <= caldate{t} < 2021 and testing_disrup_covid =1 then do; rate_1sttest = 0 ; rate_reptest = 0; end;
...
if 2020.5 <= caldate{t} < 2021 and vmmc_disrup_covid =1 then prob_circ = 0;
...
if t ge 2 and age_tm1 = 0.25 and gender=1 then do;
		if 2020.5 <= caldate{t} < 2021 and vmmc_disrup_covid =1 then prob_birth_circ = 0;	
...
if 2020.5 <= caldate{t} < 2021 and condom_disrup_covid = 1 then rred_rc = rred_rc * 1.5;
...
and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do; 
...
if pregnant=1 then do;  * dependent_on_time_step_length ;
	if a < prob_anc then do; * dec15 - remove age effect for malawi to simplify;
		...
		***LBM Aug19;
        ...
        if registd ne 1 and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do; tested=1; dt_last_test=caldate{t};np_lasttest=0; tested_anc=1;end;      
...
* if t ge 2 and gender=2 and dt_lastbirth=caldate{t}-(1/12) and tested_tm1=1 then do; 
	if registd ne 1 and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do;anc=1;tested=1;ever_tested=1; dt_last_test=caldate{t};np_lasttest=0; end;
end;
...
and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do;
	...
	if t ge 4 and caldate{t} ge date_prep_intro and hard_reach=0 and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do;
...
if prep_disrup_covid = 1 then do; 
	if 2020.5 <= caldate{t} < 2021 then do;
...
elig_test_who4=0;elig_test_who3=0;elig_test_tb=0;elig_test_who4_tested=0;elig_test_tb_tested=0;elig_test_who3_tested=0;
	if t ge 3 and registd ne 1 and tested ne 1 and caldate{t} ge date_start_testing  then do; 
		...
		if (adc_tm1=1 or (tb_tm2 =0 and tb_tm1 =1) or who3_event_tm1 =1) then do;
			unitest=uniform(0);if unitest<rate_test and (caldate{t} < 2020.5 or caldate{t} >= 2021 or testing_disrup_covid ne 1) then do;
...
* INITIATION OF ART ;
	...
	if t ge 3 and  time0 = . and (caldate{t} < 2020.5 or caldate{t} >= 2021 or (art_init_disrup_covid ne 1 and no_art_disrup_covid ne 1)) then do;
...
* INTERRUPTION OF ART  (AND LOSS TO FOLLOW-UP);
	...
	if no_art_disrup_covid = 1 then do; 
		if 2020.5 <= caldate{t} < 2021 then do;
			pre_covid_rate_int_choice = eff_rate_int_choice; 
			pre_covid_rate_restart = eff_rate_restart; 
			pre_covid_rate_return = eff_rate_return; 
			eff_rate_int_choice = 1;
			eff_rate_restart = 0; 
			eff_rate_return = 0;
		end;
		if caldate{t} ge 2021 then do;
			eff_rate_int_choice = pre_covid_rate_int_choice ; 
			eff_rate_restart = pre_covid_rate_restart ; 
			eff_rate_return = pre_covid_rate_return ; 
		end;
	end;
...
* dol;
		...
		if art_tld_eod_disrup_covid = 1 and 2020.5 <= caldate{t} < 2021 then pr_res_dol = pr_res_dol * 2; 
...
* below tox risk and persistence ;
	...
	* PCP prophylaxis; * amended nov19;
	...
	if 2020.5 <= caldate{t} < 2021 and cotrim_disrup_covid = 1 then pcp_p = 0;
...
* covid and covid death;
	...
	covid = 0; a = uniform(0);
	if age ge 15 and prev_covid ne 1 and 2020.5 <= caldate{t} < 2021 and a < 0.5 then do; covid = 1; prev_covid=1;  end; 
...
if 15 <= age or death ne . then do;
	...
	s_dead_daly + dead_daly ; s_dead_ddaly + dead_ddaly ; s_dead_allage + dead; s_death_hiv_allage + death_hiv  ;  s_death_dcause3_allage + death_dcause3 ; 
...
s_ev_onart_gt6m_vlg1000_adead  s_ev_onart_gt6m_vl_m_g1000_dead  s_ev_onart_gt6m_vl_m_g1000_adead
 s_ev_art_g1k_not2l_adead  s_dead_allage   s_death_hiv_allage  s_death_dcause3_allage 
...
art_init_disrup_covid vl_adh_switch_disrup_covid cotrim_disrup_covid no_art_disrup_covid cov_death_risk_mult
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_covid_hiv_8_4_20_8am_&dataset_id";  
put   
