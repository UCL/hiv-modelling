...
* Returning to clinic after loss to follow-up ;
	...
	if caldate{t} = 2021 and no_art_disrup_covid=1 and was_on_art_covid_disrup=1 and lost_tm1 =1 then do;
		return=1;lost=0;visit=1; 
	end;
...
if 2020.5 <= caldate{t} < 2021 and no_art_disrup_covid=1 and was_on_art_covid_disrup=1 then do;return=0;lost=1;visit=0; end;
...
* interruption due to interruption of drug supply ;
	...
	and stop_tox ne 1 then do;
		...
		if no_art_disrup_covid = 1 and 2020.5 <= caldate{t} < 2021 then do; interrupt_supply  =1; was_on_art_covid_disrup=1; end;
	...
	* people leaving clinic as well as interrupting (if interruption due to choice);
	...
	if no_art_disrup_covid = 1 and interrupt = 1 and was_on_art_covid_disrup=1 then do; lost=1;visit=0;return=0; end;
...
* RE-INITIATION OF THERAPY AFTER INTERRUPTING -  restart = 1 means restart at time t;
	...
	and tcur_tm1=. and interrupt=0 and (d < prob_supply_resumed or (no_art_disrup_covid = 1 and was_on_art_covid_disrup=1)) then do;
...
art_init_disrup_covid vl_adh_switch_disrup_covid cotrim_disrup_covid no_art_disrup_covid inc_death_rate_aids_disrup_covid cov_death_risk_mult
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_covid_hiv_12_4_20_10am_&dataset_id";  
put   
