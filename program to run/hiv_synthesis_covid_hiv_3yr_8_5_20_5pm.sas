...
* ych_risk_beh_newp;  r=uniform(0); if r < 0.50 then ych_risk_beh_newp = 0.90;	if 0.50 <= r        then ych_risk_beh_newp = 0.80;
...
* ych_risk_beh_ep;  r=uniform(0); if  r < 0.33 then ych_risk_beh_ep = 0.95; if 0.33 <= r < 0.67 then ych_risk_beh_ep = 0.90; if 0.67 <= r then ych_risk_beh_ep = 0.85; 
...
if 2020.25 <= caldate{t} < 2023.25 and swprog_disrup_covid = 1 then do;
	eff_sw_program = 0 ; 
	sw_test_6mthly = 0; 
	eff_sw_higher_int = sw_higher_int ; 
	eff_prob_sw_lower_adh = prob_sw_lower_adh ; 
...
if 2020.25 <= caldate{t} < 2023.25 and vl_adh_switch_disrup_covid = 1 then do; eff_prob_vl_meas_done=0; eff_pr_switch_line=0; end; 
...
if 2020.25 <= caldate{t} < 2023.25 and testing_disrup_covid =1 then do; rate_1sttest = 0 ; rate_reptest = 0; end;
...
if 2020.25 <= caldate{t} < 2023.25 and vmmc_disrup_covid =1 then prob_circ = 0;
...
if t ge 2 and age_tm1 = 0.25 and gender=1 then do;
		if 2020.25 <= caldate{t} < 2023.25 and vmmc_disrup_covid =1 then prob_birth_circ = 0;	
...
if 2020.25 <= caldate{t} < 2023.25 and condom_disrup_covid = 1 then rred_rc = rred_rc * 1.5;
...
and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do; 
...
if pregnant=1 then do;  * dependent_on_time_step_length ;
	if a < prob_anc then do; * dec15 - remove age effect for malawi to simplify;
		...
		***LBM Aug19;
        ...
        if registd ne 1 and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do; tested=1; dt_last_test=caldate{t};np_lasttest=0; tested_anc=1;end;      
...
* if t ge 2 and gender=2 and dt_lastbirth=caldate{t}-(1/12) and tested_tm1=1 then do; 
	if registd ne 1 and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do;anc=1;tested=1;ever_tested=1; dt_last_test=caldate{t};np_lasttest=0; end;
end;
...
and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do;
	...
	if t ge 4 and caldate{t} ge date_prep_intro and hard_reach=0 and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do;
...
if prep_disrup_covid = 1 then do; 
	if 2020.25 <= caldate{t} < 2023.25 then do;
	...
	if caldate{t} ge 2023.25 then do;
...
elig_test_who4=0;elig_test_who3=0;elig_test_tb=0;elig_test_who4_tested=0;elig_test_tb_tested=0;elig_test_who3_tested=0;
	if t ge 3 and registd ne 1 and tested ne 1 and caldate{t} ge date_start_testing  then do; 
		...
		if (adc_tm1=1 or (tb_tm2 =0 and tb_tm1 =1) or who3_event_tm1 =1) then do;
			unitest=uniform(0);if unitest<rate_test and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or testing_disrup_covid ne 1) then do;
...
if return=1 and 2020.25 <= caldate{t} < 2023.25 and no_art_disrup_covid=1 then do;return=0;lost=1;visit=0; end;
...
* INITIATION OF ART ;
	...
	if t ge 3 and  time0 = . and (caldate{t} < 2020.25 or caldate{t} >= 2023.25 or (art_init_disrup_covid ne 1 and no_art_disrup_covid ne 1)) then do;
...
* interruption due to interruption of drug supply ;
	...
	and stop_tox ne 1 then do;
		...
		if no_art_disrup_covid = 1 and 2020.25 <= caldate{t} < 2023.25 then do; interrupt_supply  =1; was_on_art_covid_disrup=1; end;
...
* RE-INITIATION OF THERAPY AFTER INTERRUPTING -  restart = 1 means restart at time t;
	...
	if caldate{t}=2023.25 and no_art_disrup_covid = 1 and was_on_art_covid_disrup=1 then do;
		restart   =1; onart   =1;tcur=0; interrupt_supply=0;
	end;
...
if art_low_adh_disrup_covid = 1 and 2020.25 <= caldate{t} < 2023.25 then adh = adh - 0.25 ;
...
* dol;
		...
		if art_tld_eod_disrup_covid = 1 and 2020.25 <= caldate{t} < 2023.25 then pr_res_dol = pr_res_dol * 2; 
...
* below tox risk and persistence ;
	...
	* PCP prophylaxis; * amended nov19;
	...
	if 2020.25 <= caldate{t} < 2023.25 and cotrim_disrup_covid = 1 then pcp_p = 0;
...
if vm ne . then do; latest_vm = vm; date_latest_vm=caldate{t}; end;
	...
	* DEATH - causes:  who4=1 / non-HIV =2 ;
	...
	dead_6m_onart=.;dead_12m_onart=.;dead_24m_onart=.;dead_36m_onart=.;
		...
		if 2020.25 <= caldate{t} < 2023.25 and inc_death_rate_aids_disrup_covid = 1 and (adc=1 or (0 <= (caldate{t} - date_most_recent_tb) <= 0.5)) then do;  
...
* covid and covid death;
	...
	covid = 0; a = uniform(0);
	if age ge 15 and prev_covid ne 1 and 2020.25 <= caldate{t} < 2023.25 then do; covid = 1; prev_covid=1;  end; 
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_covid_hiv_3yr_8_5_20_5pm_&dataset_id";  
put   
