...
cost_condoms = 0;
if option in (4 5 6) then cost_condoms = cost_condom_py;		* Fixed population-level py cost so scaling not needed;
dcost_condoms = cost_condoms * discount;


cost_fsw_services = s_sw_program_visit * cost_FSW_services_pppy * sf;
if option in (3 6) then cost_fsw_services = s_sw_program_visit * (155/1000000) * sf;
dcost_fsw_services = cost_fsw_services * discount;

cost_adh_support = 0; if option in (99) then cost_adh_support = s_diag * cost_AdhSupp_pppy * sf;* Assumes the cost is applied to everyone diagnosed;	
