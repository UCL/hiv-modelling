...
proc sort data=a.output_compressed; by run_id cald option;run;
proc freq;table run_id;where cald=2020;run;
...
keep run_id option cald dataset
...
proc sort data=y;by run_id option;run;
...
proc means  noprint data=y; var &v; output out=y_20 mean= &v._20; by run_id ; where cald = 2020; 
...
* &p ;
proc means noprint data=y; var &p ; output out=y_ mean= &p; by run_id ; where cald = 2020; run;
...
* &p ;
proc means noprint data=y; var &p ; output out=y_ mean= &p._ai1; by run_id ; where cald = 2021 and option = 1 ;run;
...
%macro par_ai0(p=);
* &p ;
proc means noprint data=y; var &p ; output out=y_ mean= &p._ai0; by run_id ; where cald = 2021 and option = 0 ;run;
