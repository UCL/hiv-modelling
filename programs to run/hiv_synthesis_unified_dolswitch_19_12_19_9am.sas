...
* circ_inc_rate; r=uniform(0); if r < 0.20 then circ_inc_rate = 0.001 ; if 0.20 <= r < 0.4  then circ_inc_rate = 0.002; if 0.4 <= r < 0.6 then 
				circ_inc_rate = 0.003;  if 0.60 <= r < 0.8  then circ_inc_rate = 0.01;  if 0.80 <= r        then circ_inc_rate = 0.03 ;
...
*			if       .  <  v{t-1} < 2.7 then rep = max(0,( 1 - (1 - tr_rate_undetec_vl)**(1/3) )+(0.000025*normal(0)));
*			else if 2.7 <= v{t-1} < 3.7 then rep = max(0,(( 1 - (1 - 0.01)**(1/3) )*fold_tr)+(0.0025*normal(0))); 
*			else if 3.7 <= v{t-1} < 4.7 then rep = max(0,(( 1 - (1 - 0.03)**(1/3) )*fold_tr)+(0.0075*normal(0)));
*			else if 4.7 <= v{t-1} < 5.7 then rep = max(0,(( 1 - (1 - 0.06)**(1/3) )*fold_tr)+(0.015*normal(0))); 
*			else if        v{t-1} >=5.7 then rep = max(0,(( 1 - (1 - 0.10)**(1/3) )*fold_tr)+(0.025*normal(0)));  
*			if primary{t-1}=1 		    then rep = max(0,( 1 - (1 - tr_rate_primary)**(1/3) )+(0.075*normal(0)));
...
* ts1m ; * replace lines above with:
*		if                   a < cu_1/cu_6 then do; * risk_nip = max(0,( 1 - (1 - tr_rate_undetec_vl)**(1/3) )+(0.000025*normal(0)));   * vl_source=1; * s_prop_rm=s_prop_vlg1_rm; * end; 
*		else if cu_1/cu_6 <= a < cu_2/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.01)**(1/3) )*fold_tr)+(0.0025*normal(0)));         * vl_source=2; * s_prop_rm=s_prop_vlg2_rm; * end; 
*		else if cu_2/cu_6 <= a < cu_3/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.03)**(1/3) )*fold_tr)+(0.0075*normal(0)));         * vl_source=3; * s_prop_rm=s_prop_vlg3_rm; * end; 
*		else if cu_3/cu_6 <= a < cu_4/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.06)**(1/3) )*fold_tr)+(0.015*normal(0)));          * vl_source=4; * s_prop_rm=s_prop_vlg4_rm; * end; 
*		else if cu_4/cu_6 <= a < cu_5/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.10)**(1/3) )*fold_tr)+(0.025*normal(0)));          * vl_source=5; * s_prop_rm=s_prop_vlg5_rm; * end; 
*		else if cu_5/cu_6 <= a < cu_6/cu_6 then do; * risk_nip = max(0,( 1 - (1 - tr_rate_primary)**(1/3) )+(0.075*normal(0)));         * vl_source=6; * s_prop_rm=s_prop_vlg6_rm; * end; 
...
* ts1m ; * replace lines above with:
*	if epvls{t}=1    then do; * risk_eip = max(0,( 1 - (1 - tr_rate_undetec_vl)**(1/3) )+(0.000025*normal(0))); * vl_source=1;  *	s_prop_rm=s_prop_vlg1_rm; * end;
*	if epvls{t} ne 1 then do;
*		if epi{t-1}=0 then do; * risk_eip = max(0,( 1 - (1 - tr_rate_primary)**(1/3) )+(0.075*normal(0))); * ep_primary=1; * vl_source=6;	* s_prop_rm=s_prop_vlg6_rm; * end;* infection in primary;
*		if epi{t-1}=1 then do; * risk_eip = max(0,(( 1 - (1 - 0.05)**(1/3) )*fold_tr)+(0.0125*normal(0))); * vl_source=4; * s_prop_rm=s_prop_vlg4_rm; * end;
*	end;* ie average risk for those with detectable vl;
...
survaids = min(dateaids, caldate&j) - infection;
aidsyn=0; if survaids=dateaids-infection > . then aidsyn=1; 

survwho34 = min(date_first_who3, tb_date, dateaids, caldate&j) - infection;
who34yn = 0; if survwho34=min(date_first_who3, tb_date, dateaid)-infection > . then who34yn =1; 
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_dolswitch_19_12_19_9am_&dataset_id";  
put   
