...
122	   tld  	remain on tle				remain on zl-pi					single VL > 1000	--> zld			    zl-pi			tl_pi
																			if adc=1 or who3=1
																			or c < 200
...
if caldate{t} ge 2019.5 and option ne 1 then reg_option = 120;
...
who may be dead and hence have caldate{t} missing;
	...
	single_vl_switch_efa_ahd_2020 = 1;
	reg_option = 122;
	time_of_first_vm_2020 = 0.5;
	min_time_repeat_vm_2020 = 0.25;
...
if caldate{t} ge 2020.5 then do;
		time_of_first_vm = time_of_first_vm_2020;
		min_time_repeat_vm = min_time_repeat_vm_2020;	
end;
...
if reg_option in (102 103 104 105 106 113 115 116 117 118 119 120 121 122) then flr=2; 
...
if reg_option in (101 102 103 104 105 106 107 109 110 111 112 113 114 115 116 117 118 119 120 121 122) then do; 
...
if reg_option in (101 102 103 104 107 110 113 116 120 121 122) then art_monitoring_strategy=150;
...
if single_vl_switch_efa_ahd_2020 = 1 then do;
art_monitoring_strategy=150;
if (o_efa=1 or (int_clinic_not_aw=1 and mr_efa=1) or o_nev=1 or (int_clinic_not_aw=1 and mr_nev=1)) and linefail=0 
and adc = 1 or who3_event = 1 or 0 < cd4 < 200 then art_monitoring_strategy=153; 
end;
...
* INITIATION OF 3rd LINE HAART - AFTER HAVING INTERRUPTED SINCE FAILING second line; * third line can now be pi after efa fail and dol fail;
	...
	if reg_option in (120 121 122) and linefail_tm1 =2 and (f_dol=1 and f_3tc=1 and (f_zdv=1 or f_ten=1)) and (p_lpr ne 1 and p_taz ne 1) then do; 
 	...
 	if reg_option in (104 118 120 121 122) and linefail_tm1=2 and (f_dol=1 and f_3tc=1 and f_ten=1) then do;
	...
	if reg_option in (103 104 110 111 114 116 117 118 119 120 121 122) and linefail_tm1=2 and (f_dol=1) then do;  * mar19 - not sure why above need f_3tc and f_ten =1 to
...
* AP 20-7-19;
	...
	* note if returing to former definition of policy 103 then need to add 103 in here;
	...
	if caldate{t} >= 2015 and (f_efa=1 or f_nev=1)  and reg_option in (103 110 111 114 116 117 119 120 121 122)  then do; * aug18;
			o_zdv=0;o_3tc=0;o_ten=0;o_nev=0;o_lpr=0;o_taz=0;o_efa=0;o_dol=0;o_dar=0;
			o_3tc=1;
 	...
 	if caldate{t} >= 2015 and f_dol=1 and reg_option in (102 103 104 113 115 116 117 118 119 120 121 122) then do;
...
if reg_option in (120 121 122) and linefail=1 and artline=2 and o_dol=1 and f_dol_tm1 ne 1 and p_taz ne 1 and p_lpr ne 1 and restart ne 1 and restart_tm1 ne 1 and t ge 2 then do; 
	if (time_since_last_vm >= 0.75) and (caldate&j - date_conf_vl_measure_done >= 1 or date_conf_vl_measure_done=.) 
and (caldate{t} - date_transition_from_pi >= 0.5 or date_transition_from_pi =.)
then do; 
		s=uniform(0);  date_last_vm_attempt=caldate&j;	if s < eff_prob_vl_meas_done then do; 
