...
* viral load changes from t-1 to t, if ART-naive at time t-1;
	...
	* dependent_on_time_step_length ;
		if naive{t}=1 or (naive{t-1}=1 and tcur{t}=0) or (toffart{t} ge 0.5) then do;
...
* due to t-1 probably * dependent_on_time_step_length ;
	...
	if  interrupt{t}=0 and o_efa_tm1=1 and transition_from_nnrti_done ne 1 and
		...
		(reg_option in (116 117) and ( sv=1 or (time_since_last_vm_gt3 = . or time_since_last_vm_gt3 > 0.75))) or
		(reg_option = 106 and sv ne 1) or 
		(reg_option in (116 117) and  (0 <= time_since_last_vm_gt3 <= 0.5))
		)
	then do;
		tss_efa   =0; o_dol=1;o_efa=0; 
		if (reg_option = 106 and sv ne 1) or (reg_option in (116 117) and  (0 <= time_since_last_vm_gt3 <= 0.5)) then do; tss_ten=0; o_ten=0;o_zdv=1;end;
...
* ts1m: *	who_risk  = 1 - exp (-(1/12)*who3_rate);
...
if vm ne . then do; latest_vm = vm; date_latest_vm=caldate{t}; end;
	...
	* DEATH - causes:  who4=1 / non-HIV =2 ;
	...
	dead_6m_onart=.;dead_12m_onart=.;dead_24m_onart=.;dead_36m_onart=.;
		...
		dwho4_rix = 1 - exp(-0.25*hiv_death_rate); 
* ts1m: *	dwho4_rix = 1 - exp (-(1/12)*hiv_death_rate);
		x3=uniform(0);
...
* ts1m :  * 	ac_death_rate = 1 - (1 - ac_death_rate)**1/3 ;
...
and incidence1549w / incidence1549m > 1.25 and incidence1549_19 < 2.5 
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_dolswitch_29_11_19_10am_&dataset_id";  
put   
