...
if t ge 2 and newp{t-1} = 0 then do;
	...
	if e < 0.6 then newp{t}=0;
	if 0.6 <= e < 0.95 then do; q=uniform(0); 
	...
	if 0.95 <= e < 0.98 then do; q=uniform(0); newp{t} = 6 + (q*34); newp{t} = round(newp{t},1);	 end;
	if 0.98 <= e < 0.99 then do; q=uniform(0); 	newp{t} = 41 + (q*89); newp{t} = round(newp{t},1);  end;
	if 0.99 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;

if  t ge 2 and 1 <= newp{t-1} <= 6 then do;
	if age{t} > 30 then e=e*0.99; * older women cant be in highest category ;
	if e < 0.1 then newp{t}=0;
	if 0.1 <= e < 0.95 then do; q=uniform(0); 
		if q < 0.7 then newp{t}=1; if 0.7 <= q < 0.8 then newp{t}=2; if 0.8 <= q < 0.9 then newp{t}=3; if 0.9 <= q < 0.95 then newp{t}=4;    
		if 0.95 <= q < 0.98 then newp{t}=5; if 0.98 <= q       then newp{t}=6;    
	end;
	if 0.95 <= e < 0.97 then do; q=uniform(0); newp{t} = 6 + (q*34); newp{t} = round(newp{t},1);	 end;
...
if  t ge 2 and 7 <= newp{t-1} <= 40 then do;
	...
	if 0.9 <= e < 0.99 then do; q=uniform(0); 	newp{t} = 41 + (q*89); newp{t} = round(newp{t},1);  end;
	if 0.99 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;

if  t ge 2 and  41 <= newp{t-1} <= 130 then do;
	if age{t} > 30 then e=e*0.95; * older women cant be in highest category ;
	if e < 0.05 then newp{t}=0;
	if 0.05 <= e < 0.1 then do; q=uniform(0); 
		if q < 0.7 then newp{t}=1; if 0.7 <= q < 0.8 then newp{t}=2; if 0.8 <= q < 0.9 then newp{t}=3; if 0.9 <= q < 0.95 then newp{t}=4;    
		if 0.95 <= q < 0.98 then newp{t}=5; if 0.98 <= q       then newp{t}=6;    
...
if  t ge 2 and  130 <= newp{t-1} then do;
	...
	if 0.1 <= e < 0.15 then do; q=uniform(0); newp{t} = 6 + (q*34); newp{t} = round(newp{t},1);	 end;
	if 0.15 <= e < 0.2 then do; q=uniform(0); 	newp{t} = 41 + (q*89); newp{t} = round(newp{t},1);  end;
	if 0.2 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
* efa;
	...
	then do;
		...
		if vm_tm1  > 3 or vm > 3 then date_start_zld_if_reg_op_116 = caldate{t};
...
if t ge 2 and o_dol = 1 and p_dol_tm1 ne 1 then date_start_dol=caldate{t};
...
and restart{t} ne 1 and restart{t-1} ne 1  and (caldate{t} - date_transition_from_nnrti >= 0.5 or date_transition_from_nnrti =.) and t ge 2 then do;  
	...
	* eee;	
	if o_dol=1 and (caldate{t} - date_conf_vl_measure_done = 0.25 and . < vm_format <= 2 and vm_tm1 gt log10(vl_threshold)) then o_dol_2nd_vlg1000 = 1;
...
*Discounting from 2020 (125th step); * 3%;           
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_dolswitch_23_11_19_8am_&dataset_id";  
put   
