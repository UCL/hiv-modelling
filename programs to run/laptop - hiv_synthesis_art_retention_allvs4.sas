...
;
  ...
  libname a 'C:\Users\Toshiba\TLO_HMC Dropbox\Andrew Phillips\My PC (LAPTOP-ANT5JKE9)\Documents\My SAS Files\outcome model\misc\';
...
tr_rate_undetec_vl = 0.000; * allvs4;
...
test_rate_non_tb_who3=0.05; * dependent_on_time_step_length ;
...
prob_lossdiag_non_tb_who3e = 0.15; *overwritten;
...
* AP 19-7-19 ;  * changed for art_retention joint modelling analysis - this is not the same distribution as in base program;
* rate_int_choice;  r=uniform(0);   if 0.0  <= r < 0.25 then rate_int_choice = 0.002; 
			if 0.25 <= r < 0.50 then rate_int_choice = 0.004; if 0.50 <= r < 0.75 then rate_int_choice = 0.008 ;
			if 0.75 <= r  then rate_int_choice = 0.015 ;
...
* can add in potential changes in policy after 2020.5 here -  set specific '2020' variables to '1' and specify proportion of
  runs for which the option should take place;
	...
	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 0.25) ; 
	eff_rate_lost = eff_rate_lost / 1.333 ; 
	eff_prob_lost_art = eff_prob_lost_art / 1.333 ; 
 	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 0.25) ; 
	eff_rate_int_choice = eff_rate_int_choice / 1.333 ; 
 	eff_prob_return_adc = eff_prob_return_adc + ((1 - eff_prob_return_adc) * 0.25) ; 
 	eff_rate_return = eff_rate_return + ((1 - eff_rate_return) * 0.25) ; 
	...
	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 0.75) ; 
	eff_rate_lost = eff_rate_lost / 4     ; 
	eff_prob_lost_art = eff_prob_lost_art / 4     ; 
 	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 0.75) ; 
	eff_rate_int_choice = eff_rate_int_choice / 4     ; 
 	eff_prob_return_adc = eff_prob_return_adc + ((1 - eff_prob_return_adc) * 0.75) ; 
 	eff_rate_return = eff_rate_return + ((1 - eff_rate_return) * 0.75) ; 

	end;

	if option = 4 then do;

	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 1   ) ; 
	eff_rate_lost = eff_rate_lost / 100000 ; 
	eff_prob_lost_art = eff_prob_lost_art / 100000 ; 
 	eff_rate_restart = eff_rate_restart + ((1 - eff_rate_restart) * 1) ; 
	eff_rate_int_choice = eff_rate_int_choice / 100000 ; 
...
*allvs4;
if option=4 then do; epdiag=1; epart=1; epvls=1; end;


* if epart ne 1 then epvls=0;
...
if t ge 2 and nip gt 0 then do;
	d=1;do until (d gt nip);
		...
		a=uniform(0);
		if                   a < cu_1/cu_6 then do; risk_nip = max(0,tr_rate_undetec_vl+(0.000025*normal(0))) ; vl_source=1; t_prop_rm=t_prop_vlg1_rm; end; *new for prep;
...
linefail=0; artline=1; tcur   =0; cd4_tcur0 = cd4; line1=1;vfail1=0; naive=0; o_3tc=1; o_ten=1; o_dol=1; art_initiation=1;
...
hivge15=0; if hiv=1 and 15 <= age then hivge15=1;
...
vl50=.; vg50=.; vl200=.; vg200=.; vl1000= .; vg1000=.; vg1000_m=.; vg1000_w=.; vg1000_m_1524=.;vg1000_w_1524=.; vl500=.;
...
if hiv =1 then do;
	...
	vg200=1-vl200;
		if . < vl < 2.7 then vl500=1; else vl500=0;
	vg500=1-vl500;
...
*Discounting from 2021 (125th step); * 3%;  * dependent_on_time_step_length ;         
d=&j;  
discount = (0.9924141173)**(d-129);
* ts1m:  discount = (0.9974649513861632)**(d-(129*3));
...
adclam_measured_this_per_adc = 0; adc_proph_adc = 1;
if adc=1 then do;
...
crag_measured_this_per_crypm = 0; crypm_proph_crypm = 1;
if crypm=1 then do;
...
if 15 <= age < 65 and (death = . or caldate&j = death ) then do;
	...
	s_restart + restart;  s_art_initiation + art_initiation;
...
if 15 <= age and (death = . or caldate&j = death ) then do; 
	...
	s_alive_w + alive_w ; s_alive_m + alive_m ; s_hivge15 + hivge15 ;
...
proc freq; tables cald hiv primary ; run; 

proc means; var vl1000 vl500 s_epvls  s_epart; where hiv =1 and death=.; run;
...
* where hiv_tm1 ne 1 and death = . and 15 <= age < 30 and serial_no < 5000 and (nip ge 1 or epi=1);
run;



/*
proc freq; tables infected_primary ; 
where primary=1; 
proc freq; tables infection; 
run;
*/
