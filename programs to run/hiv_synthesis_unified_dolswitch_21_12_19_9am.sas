...
- 11_12_19 8pm - correct {1-1} and change (0.25 <= tcur{t-1} < 0.5 or (tcur{t-1} >= 0.5  and v{t-1} ge 4))

- 4_12_19 3pm -  see loveleen changes below

- 3_12_19 4pm  - adjust range of fx, dist of gx, add in v{t}=vmax{t-1} from 3 months after interruption
...
- 16-11-19 3pm -  change if registd{t}=1 and tested{t} ne 1 and u<0.7 then do  tested{t}=1  tested_anc_prevdiag=1  end   to remove tested{t}=1 so cant
...
- 05-10-19 12pm - added prep_strategy to outputs, define newp_this_per_art_or_prep etc and add to outputs, 
				  added: if t ge 2 and onart{t-1}=1 and (prep ne 1 or adh{t} = .), changed dn of adh_pattern_prep
...
20-12-19 - circ_inc_rate distribution widened to include more values at upper end. 
...
circ_inc_rate = 0.0015; *overwritten; * dependent_on_time_step_length ;
...
adc_cost_a  =  adc_cost_a  / 3;
who3_cost_a  = who3_cost_a   / 3;
...
if sw1 = 1 then do;
e=uniform(0); if age1 > 30 then e=e*0.99; * older women cant be in highest category ;
...
if t ge 2 and caldate{t-1} < 2070.0  and dead{t-1} ne 1 and dead{t-1} ne .  then caldate{t}=caldate{t-1}+0.25; * dependent_on_time_step_length ;
* ts1m ; * change this line to: 
...
* INTERVENTIONS / CHANGES in 2020.5;

* insert policy-specific code in here: ;

option = &s;

if option = 1 and caldate{t} ge 2019.5 then reg_option = 113;
if option = 2 then reg_option = 103;
if option = 3 then reg_option = 116;
if option = 4 then reg_option = 104;


option_0_prep_continue_2020=1;

* this below is just so dont need to delete these parameter names below;
...
age2=age1+0.25;  * dependent_on_time_step_length ;
* ts1m ; * change this line to: 
age2=age1 + (1/12);
;
...
* risk of having sti;
sti{t}=0;
if t ge 2 and  sti{t-1}=0 then  do; * dependent_on_time_step_length ;
...
if sex_beh_trans_matrix_m=1 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.995; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.00005; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.00005; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.03; *newp=1; s2=0.07; *newp >= 2; s3=0.90; *newp 10x; s4=0.00025; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.05; *newp 10x; s4=0.9500; end;
end;
if sex_beh_trans_matrix_m=2 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.98; *newp=1; s2=0.01 ; *newp >= 2; s3=0.01 ; *newp 10x; s4=0.00025; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.98; *newp=1; s2=0.01; *newp >= 2; s3=0.01; *newp 10x; s4=0.00025; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.05; *newp=1; s2=0.15; *newp >= 2; s3=0.80; *newp 10x; s4=0.00125; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.20; *newp 10x; s4=0.8000; end;
end;
if sex_beh_trans_matrix_m=3 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.0005 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.93; *newp=1; s2=0.05; *newp >= 2; s3=0.02; *newp 10x; s4=0.0005 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.0025 ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.40; *newp 10x; s4=0.6000; end;
end;
if sex_beh_trans_matrix_m=4 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.995; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.0001 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.0001 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.03; *newp=1; s2=0.07; *newp >= 2; s3=0.90; *newp 10x; s4=0.0005  ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.04; *newp=1; s2=0.04; *newp >= 2; s3=0.09; *newp 10x; s4=0.8300; end;
end;
if sex_beh_trans_matrix_m=5 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.98; *newp=1; s2=0.01 ; *newp >= 2; s3=0.01 ; *newp 10x; s4=0.005  ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.98; *newp=1; s2=0.01; *newp >= 2; s3=0.01; *newp 10x; s4=0.0005 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.05; *newp=1; s2=0.15; *newp >= 2; s3=0.80; *newp 10x; s4=0.0025 ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.025; *newp=1; s2=0.06; *newp >= 2; s3=0.17; *newp 10x; s4=0.7500; end;
end;
if sex_beh_trans_matrix_m=6 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.001 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.93; *newp=1; s2=0.05; *newp >= 2; s3=0.02; *newp 10x; s4=0.001 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.005  ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.04; *newp=1; s2=0.08; *newp >= 2; s3=0.21; *newp 10x; s4=0.6700; end;
end;
if sex_beh_trans_matrix_m=7 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.995; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.000025; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.000025; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.03; *newp=1; s2=0.07; *newp >= 2; s3=0.90; *newp 10x; s4=0.000125; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.05; *newp 10x; s4=0.9500; end;
end;
if sex_beh_trans_matrix_m=8 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.98; *newp=1; s2=0.01 ; *newp >= 2; s3=0.01 ; *newp 10x; s4=0.000125; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.98; *newp=1; s2=0.01; *newp >= 2; s3=0.01; *newp 10x; s4=0.000125; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.05; *newp=1; s2=0.15; *newp >= 2; s3=0.80; *newp 10x; s4=0.000625; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.20; *newp 10x; s4=0.8000; end;
end;
if sex_beh_trans_matrix_m=9 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.02; *newp 10x; s4=0.00025 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.93; *newp=1; s2=0.05; *newp >= 2; s3=0.02; *newp 10x; s4=0.00025 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.00125 ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.40; *newp 10x; s4=0.6000; end;
end;
if sex_beh_trans_matrix_m=10 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.90; *newp=1; s2=0.06; *newp >= 2; s3=0.04; *newp 10x; s4=0.0005 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.99; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.0005 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.0025 ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.40; *newp 10x; s4=0.6000; end;
end;
if sex_beh_trans_matrix_m=11 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.90; *newp=1; s2=0.06; *newp >= 2; s3=0.04; *newp 10x; s4=0.001 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.99; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.001 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.005  ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.04; *newp=1; s2=0.08; *newp >= 2; s3=0.21; *newp 10x; s4=0.6700; end;
end;
if sex_beh_trans_matrix_m=12 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.90; *newp=1; s2=0.06; *newp >= 2; s3=0.04; *newp 10x; s4=0.00025 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.99; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.00025 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.20; *newp=1; s2=0.20; *newp >= 2; s3=0.60; *newp 10x; s4=0.00125 ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.00; *newp=1; s2=0.00; *newp >= 2; s3=0.00; *newp 10x; s4=1.0000; end;
end;
if sex_beh_trans_matrix_m=13 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.75; *newp=1; s2=0.15; *newp >= 2; s3=0.10; *newp 10x; s4=0.0005 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.99; *newp=1; s2=0.005; *newp >= 2; s3=0.005; *newp 10x; s4=0.0005 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.90; *newp=1; s2=0.05; *newp >= 2; s3=0.03; *newp 10x; s4=0.02   ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.90; *newp=1; s2=0.05; *newp >= 2; s3=0.03; *newp 10x; s4=0.02  ; end;
end;
if sex_beh_trans_matrix_m=14 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.75; *newp=1; s2=0.15; *newp >= 2; s3=0.10; *newp 10x; s4=0.001 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.99; *newp=1; s2=0.05; *newp >= 2; s3=0.02; *newp 10x; s4=0.001 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.01; *newp 10x; s4=0.01   ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.95; *newp=1; s2=0.03; *newp >= 2; s3=0.01; *newp 10x; s4=0.0100; end;
end;
if sex_beh_trans_matrix_m=15 then do;
if       newp{t-1}=0  then do; *newp=0; s1=0.75; *newp=1; s2=0.15; *newp >= 2; s3=0.10; *newp 10x; s4=0.00025 ; end;
if newp{t-1} = 1  then do; *newp=0; s1=0.93; *newp=1; s2=0.05; *newp >= 2; s3=0.02; *newp 10x; s4=0.00025 ; end;
if 2  <= newp{t-1}<4 then do; *newp=0; s1=0.80; *newp=1; s2=0.10; *newp >= 2; s3=0.05; *newp 10x; s4=0.05    ; end;
if 4 <= newp{t-1}    then do; *newp=0; s1=0.80; *newp=1; s2=0.10; *newp >= 2; s3=0.05; *newp 10x; s4=0.05  ; end;
end;
...
if sex_beh_trans_matrix_w=1 then do;
	...
	if 1  <= newp{t-1}     then do; *newp=0; s1=0.99; *newp=1-3; s2=0.01; end;
end;
if sex_beh_trans_matrix_w=2 then do;
	if       newp{t-1}=0   then do; *newp=0; s1=0.995; *newp=1-3; s2=0.005; end;
	if 1  <= newp{t-1}     then do; *newp=0; s1=0.98; *newp=1-3; s2=0.02; end;
end;
if sex_beh_trans_matrix_w=3 then do;
	if       newp{t-1}=0   then do; *newp=0; s1=0.995; *newp=1-3; s2=0.005; end;
	if 1  <= newp{t-1}     then do; *newp=0; s1=0.95; *newp=1-3; s2=0.05; end;
end;
if sex_beh_trans_matrix_w=4 then do;
	if       newp{t-1}=0   then do; *newp=0; s1=0.995; *newp=1-3; s2=0.005; end;
	if 1  <= newp{t-1}     then do; *newp=0; s1=0.85; *newp=1-3; s2=0.15; end;
end;
if sex_beh_trans_matrix_w=5 then do;
	if       newp{t-1}=0   then do; *newp=0; s1=0.995; *newp=1-3; s2=0.005; end;
	if 1  <= newp{t-1}     then do; *newp=0; s1=0.75; *newp=1-3; s2=0.25; end;
end;
...
if t ge 2 and newp{t-1} = 0 then do;
	if age{t} > 30 then e=e*0.99; * older women cant be in highest category ;
	...
	if sw_newp_lev_1_1+sw_newp_lev_1_2+sw_newp_lev_1_3+sw_newp_lev_1_4 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
if  t ge 2 and 1 <= newp{t-1} <= 6 then do;
	...
	if sw_newp_lev_2_1+sw_newp_lev_2_2+sw_newp_lev_2_3+sw_newp_lev_2_4 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
if  t ge 2 and 7 <= newp{t-1} <= 40 then do;
	...
	if sw_newp_lev_3_1+sw_newp_lev_3_2+sw_newp_lev_3_3+sw_newp_lev_3_4 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;

if  t ge 2 and  41 <= newp{t-1} <= 130 then do;
	if age{t} > 30 then e=e*0.98; * older women cant be in highest category ;
	...
	if sw_newp_lev_4_1+sw_newp_lev_4_2+sw_newp_lev_4_3+sw_newp_lev_4_4  <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
if  t ge 2 and  130 <  newp{t-1} then do;
	...
	if sw_newp_lev_5_1+sw_newp_lev_5_2+sw_newp_lev_5_3+sw_newp_lev_5_4 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
* ts1m; newp{t}=newp{t}/3; newp{t} = round(newp{t},1);  end;
end;
...
ep{t}=0; * dependent_on_time_step_length ;
...
if ep{t}=1 then do; epmono=0;
	...
	if epmono=1 and epi{t} ne 1 then do;
		if hiv{t}=1 then do;  * rep * dependent_on_time_step_length ;
...
if gender=2 and t ge 4 and ((caldate{t}-dt_lastbirth gt 1.25) or dt_lastbirth=.) then do;
	...
	if pregnant ne 1 and newp{t-3} gt 1 then do; * dependent_on_time_step_length ;
...
if t ge 2 and gender=2 and dt_lastbirth=caldate{t}-0.25 and tested{t-1}=1 then do; * dependent_on_time_step_length ;
	...
	if prep_ever=1 and dt_prep_s ne caldate{t} and (tested{t} ne 1 or (tested{t}=1 and (hiv{t}=0 or (hiv{t}=1 and unisensprep > sens_vct)))) then do; * may17;
		if prep_tm1 = 1 then do; * dependent_on_time_step_length;
			...
			*dt_prep_rs = date of prep restart (to count number of prep re-initiations);
			...
			if tested{t}=1 then do; * dependent_on_time_step_length;
...
if pop_wide_tld = 1 and registd{t} ne 1 and ( prep_elig = 1 or ( ever_newp = 1 and ever_tested ne 1 ) ) then do;  
	...
	if prep_ever ne 1 then do;   * dependent_on_time_step_length; 
			r=uniform(0); if (prep_willing_pop=1 or  (sw{t}=1 and prep_willing_sw=1 )) and r < prob_prep_pop_wide_tld then do ;
...
* ts1m ; * replace lines above with:
*		if                   a < cu_1/cu_6 then do; * risk_nip = max(0,( 1 - (1 - tr_rate_undetec_vl)**(1/3) )+(0.000025/3*normal(0)));   * vl_source=1; * s_prop_rm=s_prop_vlg1_rm; * end; 
*		else if cu_1/cu_6 <= a < cu_2/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.01)**(1/3) )*fold_tr)+(0.0025/3*normal(0)));         * vl_source=2; * s_prop_rm=s_prop_vlg2_rm; * end; 
*		else if cu_2/cu_6 <= a < cu_3/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.03)**(1/3) )*fold_tr)+(0.0075/3*normal(0)));         * vl_source=3; * s_prop_rm=s_prop_vlg3_rm; * end; 
*		else if cu_3/cu_6 <= a < cu_4/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.06)**(1/3) )*fold_tr)+(0.015/3*normal(0)));          * vl_source=4; * s_prop_rm=s_prop_vlg4_rm; * end; 
*		else if cu_4/cu_6 <= a < cu_5/cu_6 then do; * risk_nip = max(0,(( 1 - (1 - 0.10)**(1/3) )*fold_tr)+(0.025/3*normal(0)));          * vl_source=5; * s_prop_rm=s_prop_vlg5_rm; * end; 
*		else if cu_5/cu_6 <= a < cu_6/cu_6 then do; * risk_nip = max(0,( 1 - (1 - tr_rate_primary)**(1/3) )+(0.075/3*normal(0)));         * vl_source=6; * s_prop_rm=s_prop_vlg6_rm; * end; 
...
of transmission.  if so, the tr_rate_primary should be lowered;
		...
		if prep   =1 then do; 
			...
			if m184m_p=1 and k65m_p=1 and (inpm_p ne 1 and pop_wide_tld_prep=1)  then risk_nip = risk_nip * (1-(adh{t} * eff_adh_prep));
			if m184m_p=1 and k65m_p=1 and inpm_p = 1 and pop_wide_tld_prep=1  then risk_nip = risk_nip * (1-(adh{t} * 0.5 * eff_adh_prep));
...
d=uniform(0);
if epi{t}=1 then do;  * dependent_on_time_step_length ;  
...
*	end;* ie average risk for those with detectable vl;
	...
	if gender=1 and mcirc   =1   then risk_eip = risk_eip* 0.4;  * lower transmission risk in men circumcised;
		...
		if prep   =1 then do; 
			if m184m_p ne 1 and k65m_p ne 1 and tam_p<3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p ne 1 and k65m_p ne 1 and tam_p>=3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p=1 and k65m_p ne 1 and tam_p<3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p ne 1 and k65m_p=1 and tam_p<3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p=1 and k65m_p ne 1 and tam_p>=3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p ne 1 and k65m_p=1 and tam_p>=3 then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p=1 and k65m_p=1  then risk_eip = risk_eip * (1-(adh{t} * 0.50 * eff_adh_prep));
			if m184m_p=1 and k65m_p=1 and (inpm_p ne 1 and pop_wide_tld_prep=1)  then risk_eip = risk_eip * (1-(adh{t} * eff_adh_prep));
			if m184m_p=1 and k65m_p=1 and inpm_p = 1 and pop_wide_tld_prep=1  then risk_eip = risk_eip * (1-(adh{t} * 0.5 * eff_adh_prep));
...
array visit{8} visit&g-visit&h; * currently under hiv care ;
...
array resumec{8} resumec&g-resumec&h; * indicates if cd4 has reached cmin after interruption and before toffart= 1 (ie 1 year);
...
* VISIT FREQUENCY  ;
	visit{t}=visit{t-1};  * dependent_on_time_step_length ;  
	if lost{t-1}=1 then visit{t}=0; * dependent_on_time_step_length ;  

	vfail1_{t}=vfail1_{t-1};

	onart{t}=onart{t-1};    naive{t}=naive{t-1};    artline{t}=artline{t-1};
	if onart{t}=1 then tcur{t}=tcur{t-1}+0.25;   
* ts1m:  	if onart{t}=1 then tcur{t}=tcur{t-1} + (1/12) ;
	if prep   =1 then tcur{t}=tcur{t-1}+0.25;   
* ts1m:  	if prep   =1 then tcur{t}=tcur{t-1} + (1/12) ;
...
* viral load changes from t-1 to t, if ART-naive at time t-1;
	...
	* dependent_on_time_step_length ;
		if naive{t}=1 or (naive{t-1}=1 and tcur{t}=0) or (toffart{t} gt 0.25) then do;
...
- cc{t-1} is change from t-1 to t;
	...
	if t ge 4 and 0 < toffart{t} <= 0.25 then do;
    	 v{t}=vmax{t-1};
...
* ts1m:  cc{t-1} = cc{t-1} / 3;
	     z=c{t-1}+cc{t-1};
	     c{t}=max(cmin{t-1},z); if c{t} lt 0 then c{t}=0; if c{t}=cmin{t-1} then resumec{t}=1; 
	end;
	if t ge 4 and 0.25 < toffart{t} <= 0.5 and resumec{t-1} ne 1 then do;
	     * assume rate cd4 fall depends on current viral load;
...
* ts1m:  cc{t-1} = cc{t-1} / 3;
	...
	if 0.5 < toffart{t} <= 0.75 and t ge 4 and (resumec{t-1} ne 1 and resumec{t-2} ne 1) then do;
	     * assume rate cd4 fall depends on current viral load;
...
* ts1m:  cc{t-1} = cc{t-1} / 3;
	...
	c_200_gt_nad=0;if t ge 3 and toffart{t} gt 0 and c{t-1}-cmin{t-1} > 300 then do;
    	 * assume rate cd4 fall depends on current viral load;
	     c_200_gt_nad=1;
    	 if v{t} >= 5 then cc{t-1}=-200+10*normal(0);
	     if 4.5 <= v{t} < 5 then cc{t-1}=-160+10*normal(0);
...
if reg_option in (103 104 110 111 114 116 117 118 119) and linefail{t-1}=2 and (f_dol=1) then do;  * mar19 - not sure why above need f_3tc and f_ten =1 to
...
* adherence between t-1 and t  (adh{t}); 
	...
	if t ge 2 and onart{t-1}=1 and (prep ne 1 or adh{t} = .) then do;  * for person on prep whether pop wide tld or regular prep the adh should be
...
* ts1m:  if t ge 2 and tcur{t-1}=0 and caldate{t} = yrart + (1/12) then adh_in_first_period_onart = adh{t};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
 		if 0 <= tcur{t-1} < 0.25 then do;
 			 if adh{t} >= 0.8 then do;   v{t}=vmax{t-1}-(3.0/3)+(sd_v_art*normal(0));   vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+180);    newmut{t-1}= 0.002*((v{t}+v{t-1})/2);  end;   
 			 if 0.5 <= adh{t} < 0.8 then do;   v{t}=vmax{t-1}-(2.0/3)+(sd_v_art*normal(0));   vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30 );   newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 			 if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.5/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.15*((v{t}+v{t-1})/2); end;
 		end;
 		if tcur{t-1} = 1/12 then do;
 			 if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(3.0/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+180);  newmut{t-1}= 0.002*((v{t}+v{t-1})/2);  end;   
 			 if 0.5 <= adh{t} < 0.8 then do; v{t}=vmax{t-1}-(2.0/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30 );  newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 			 if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.5/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.15*((v{t}+v{t-1})/2); end;
 		end;
 		if tcur{t-1} = 2/12 then do;
 			 if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-3.0+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+180);  newmut{t-1}= 0.002*((v{t}+v{t-1})/2);  end;   
 			 if 0.5 <= adh{t} < 0.8 then do; v{t}=vmax{t-1}-2.0+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			 cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30 );  newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
...
* nactive ge 3 - 3-6 months; * ( cd4 change and newmut depend only on most recent adh - vl depends on most recent and previous);
		if  (0.25 <= tcur{t-1} < 0.5 or (tcur{t-1} >= 0.5  and v{t-1} ge 4)) and t ge 2 then do;
...
* nactive ge 3 - GE 6 months;
		if (tcur{t-1} ge 0.5 and v{t-1} lt 4) then do;
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
  		if 0 <= tcur{t-1} < 0.25 then do;
 			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(2.6/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
  		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+80);   newmut{t-1}= 0.01*((v{t}+v{t-1})/2);  end;   
 			if 0.5 <= adh{t} < 0.8 then do; v{t}=vmax{t-1}-(1.6/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30 );  newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.4/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+4 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 		end;
 			if tcur{t-1}=1/12 then do;
 			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-(2.6/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+80);  newmut{t-1}= 0.01*((v{t}+v{t-1})/2);  end;    
 			if 0.5 <= adh{t} < 0.8 then do; v{t}=vmax{t-1}-(1.6/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30 );  newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.4/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+4 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.15*((v{t}+v{t-1})/2);  end;
 		end;
 			if tcur{t-1}=2/12 then do;
 			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-2.6+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
 		if 0 <= tcur{t-1} < 0.25 then do;
 		if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-(2.2/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+40);   newmut{t-1}= 0.03*((v{t}+v{t-1})/2);   end;
 			if 0.5 <= adh{t} < 0.8 then do;  v{t}=vmax{t-1}-(1.2/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+23 );  newmut{t-1}=0.2*((v{t}+v{t-1})/2);   end;
 			if adh{t} < 0.5 then do;    v{t}=vmax{t-1}-(0.3/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+3 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.2*((v{t}+v{t-1})/2);   end;
 		end;
 		if tcur{t-1}=1/12 then do; 
		if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-(2.2/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+40);  newmut{t-1}= 0.03*((v{t}+v{t-1})/2);  end;
 			if 0.5 <= adh{t} < 0.8 then do; v{t}=vmax{t-1}-(1.2/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+23 );  newmut{t-1}=0.2*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.3/2)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+3 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.2*((v{t}+v{t-1})/2);   end;
 		end;
 		if tcur{t-1}=2/12 then do; 
 			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-2.2+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
  			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-(1.8/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+35);   newmut{t-1}= 0.05*((v{t}+v{t-1})/2);   end;
 			if 0.5 <= adh{t} < 0.8 then do;  v{t}=vmax{t-1}-(1.1/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+20 );   newmut{t-1}=0.25*((v{t}+v{t-1})/2);   end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.25/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+2 );   newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.25*((v{t}+v{t-1})/2);  end;
 		end;
 			if tcur{t-1}=1/12 then do; 
  			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-(1.8/2)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+35);  newmut{t-1}= 0.05*((v{t}+v{t-1})/2);  end;
 			if 0.5 <= adh{t} < 0.8 then do;  v{t}=vmax{t-1}-(1.1/2)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+20 );  newmut{t-1}=0.25*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.25/2)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+2 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);   if (o_nev=1 or o_efa=1) then newmut{t-1}=0.25*((v{t}+v{t-1})/2);  end;
 		end;
 			if tcur{t-1}=2/12 then do; 
  			if adh{t} >= 0.8 then do;  v{t}=vmax{t-1}-1.8+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
 		if 0 <= tcur{t-1} < 0.25 then do;
		 	if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - (1.5/3)+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1}; 
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30); newmut{t-1}= 0.1*((v{t}+v{t-1})/2);  end;
 			if 0.5 <= adh{t} < 0.8 then do; v{t}= vmax{t-1} - (0.9/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+15 );  newmut{t-1}=0.3*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;   v{t}=vmax{t-1}-(0.2/3)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+1 );  newmut{t-1}=0.05*((v{t}+v{t-1})/2);  if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2);  end;
 		end;
 		if tcur{t-1}=1/12 then do; 
		 	if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - 1.5/2+(sd_v_art*normal(0)); vc{t-1}=v{t}-v{t-1}; 
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+30);  newmut{t-1}= 0.1*((v{t}+v{t-1})/2);  end;
 			if 0.5 <= adh{t} < 0.8 then do; v{t}= vmax{t-1} - (0.9/2)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+15 );  newmut{t-1}=0.3*((v{t}+v{t-1})/2);  end;
 			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}-(0.2/2)+(sd_v_art*normal(0));  vc{t-1}=v{t}-v{t-1};
 		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+1 );   newmut{t-1}=0.05*((v{t}+v{t-1})/2);   if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2);  end;
 		end;
 		if tcur{t-1}=2/12 then do; 
 			if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - 1.5+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
			if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - (1.25/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+25); newmut{t-1}= 0.15*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}= vmax{t-1} - (0.8/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+13 ); newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}-(0.15/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-1 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
			if tcur{t-1}=1/12 then do; 
			if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - (1.25/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+25); newmut{t-1}= 0.15*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}= vmax{t-1} - (0.8/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
			cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+13 ); newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}-(0.15/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-1 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
			if tcur{t-1}=2/12 then do; 
			if adh{t} >= 0.8 then do; v{t}= vmax{t-1} - 1.25+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
 			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.9/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+20); newmut{t-1}= 0.2*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.6/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+10 ); newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}-0.0+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-3 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
		if tcur{t-1}=1/12 then do; 
 			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.9/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+20); newmut{t-1}= 0.2*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.6/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+10 ); newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}-0.0+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-3 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.8/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+17); newmut{t-1}= 0.3*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.5/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+8  ); newmut{t-1}=0.35*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.05/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-6 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
			if tcur{t-1}=1/12 then do; 
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.8/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+17); newmut{t-1}= 0.3*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.5/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+8  ); newmut{t-1}=0.35*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.05/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-6 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.3*((v{t}+v{t-1})/2); end;
		end;
			if tcur{t-1}=2/12 then do; 
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-0.8+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
		 	if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.7/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+13); newmut{t-1}= 0.4*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.4/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5 ); newmut{t-1}=0.4*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-10 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2);if (o_nev=1 or o_efa=1) then newmut{t-1}=0.4*((v{t}+v{t-1})/2);  end;
		end;
		if tcur{t-1}=1/12 then do; 
		 	if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.7/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+13); newmut{t-1}= 0.4*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.4/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5 ); newmut{t-1}=0.4*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-10 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2);if (o_nev=1 or o_efa=1) then newmut{t-1}=0.4*((v{t}+v{t-1})/2);  end;
		end;
		if tcur{t-1}=2/12 then do; 
		 	if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-0.7+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.55/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+10 ); newmut{t-1}= 0.45*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.25/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+3 ); newmut{t-1}=0.45*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-11 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.45*((v{t}+v{t-1})/2); end;
		end;
		if tcur{t-1}=1/12 then do; 
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.55/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+10 ); newmut{t-1}= 0.45*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.25/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+3 ); newmut{t-1}=0.45*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-11 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.45*((v{t}+v{t-1})/2); end;
		end;
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.4/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5); newmut{t-1}= 0.5*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.1/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+0 ); newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-12 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2);if (o_nev=1 or o_efa=1) then newmut{t-1}=0.5*((v{t}+v{t-1})/2);  end;
		end;
		if tcur{t-1}=1/12 then do; 
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.4/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+5); newmut{t-1}= 0.5*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.1/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(pt_cd4_rise_art*+0 ); newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-12 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2);if (o_nev=1 or o_efa=1) then newmut{t-1}=0.5*((v{t}+v{t-1})/2);  end;
		end;
...
* ts1m - replace above single block with three blocks below: (note that newmut and cc are divided by 3 lower down for ts1m)
		if 0 <= tcur{t-1} < 0.25 then do;
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.3/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
    		cc{t-1}=cd4_art_adj+(-2); newmut{t-1}= 0.5*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.05/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-7 ); newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/3)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-13 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
		end;
		if tcur{t-1}=1/12 then do; 
			if adh{t} >= 0.8 then do; v{t}=vmax{t-1}-(0.3/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
    		cc{t-1}=cd4_art_adj+(-2); newmut{t-1}= 0.5*((v{t}+v{t-1})/2); end;
			if 0.5 <= adh{t} < 0.8 then do;v{t}=vmax{t-1}-(0.05/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-7 ); newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
			if adh{t} < 0.5 then do;  v{t}=vmax{t-1}+(0.1/2)+(sd_v_art*normal(0));vc{t-1}=v{t}-v{t-1};
		    cc{t-1}=cd4_art_adj+(-13 ); newmut{t-1}=0.05*((v{t}+v{t-1})/2); if (o_nev=1 or o_efa=1) then newmut{t-1}=0.5*((v{t}+v{t-1})/2); end;
		end;
...
then do; 
	...
	* eee;	
	...
	(caldate{t}=date_conf_vl_measure_done and vm_format in (3,4) and vm gt log10(vl_threshold) and adh{t} > 0.8) 
	or
	(caldate{t} - date_conf_vl_measure_done = 0.25 and . < vm_format <= 2 and value_last_vm gt log10(vl_threshold) and adh{t-1} > 0.8)
	) 
	then do;
			linefail{t}=2;r_fail_2=c_totmut{t}; cd4_fail1_2=c{t}; vl_fail_2=v{t}; d2ndlfail=caldate{t}; 
			if o_zdv=1 then f_zdv=1;
...
* below tox risk and persistence ;
	...
	* note that increased mortality risk due to weight rise on dol is independent of weightg as a tox (potentially leading to discontinuation)
	...
	if t ge 2 and o_dol=1 then do;  * take incidence as half for efa but with same persistence rate; * apr16; * changed since nih talk; 
		if c_cns_tm1=0 and rr < (r_cns_start_dol * rel_dol_tox) and tcur{t} < 1 then c_cns=1;
		if c_cns_tm1=1 then c_cns=1; if c_cns_tm1=1  and tcur{t} < 1 and rr < p_cns_stops_dol then c_cns=0;
		if c_cns_tm1=1 then c_cns=1; if c_cns_tm1=1  and tcur{t} >= 1 and rr < p_cns_stops_dol / 6 then c_cns=0; * this makes this p_cns_stops the same as for efa;
	end;
...
if prep=1 and pop_wide_tld_prep ne 1 then do;
	...
	cost_prep = prep_drug_cost ;  cost_prep_ac_adh=cost_prep*adh{t}; * 1.2 is supply chain, as for arvs;
...
if pop_wide_tld_prep = 1 then do;
	...
	cost_prep = prep_drug_cost_tld ;  cost_prep_ac_adh=cost_prep*adh{t}; * 1.2 is supply chain, as for arvs;
	if visit_prep = 1 then cost_prep_visit = cost_prep_clinic / 2; * drug pick-up only - mar18 ; 
	if visit_prep = 2 then cost_prep_visit = cost_prep_clinic; 
	if visit_prep = 3 then cost_prep_visit = cost_prep_clinic+cost_prep_clinic_couns;
end;
...
*** Distribution of resistance mutations amongst those with resistance to inform rm in the infectious pool;
	...
	if rm_&j=1 then do;
		...
		p54m_&j=0; if c_pr54m=1  then p54m_&j=1;
	...
	if primary&j=1 then do; r_&j=0;
		...
		or r_dar >= 0.5 or r_lpr >= 0.5  or r_taz >= 0.5 or r_dol >= 0.5 then r_&j=1;
	end;
...
onart_gt6m_nnres_vlg1000=0; if ((onart&j=1 and tcur&j ge 0.5) or int_clinic_not_aw=1) and vl1000=0 and (c_rt103m=1 or c_rt181m=1 or c_rt190m=1)  and caldate&j ge yrart+0.5 > .
...
newp_this_per_art_or_prep   newp_this_per_art   newp_this_per_prep  newp_this_per
...
r_&j	 r_3tc  r_nev  r_lpr   r_taz   r_efa   r_ten   r_zdv r_dol
...
o_zdv_tox o_3tc_tox o_ten_tox o_taz_tox o_lpr_tox o_efa_tox o_nev_tox o_dol_tox 
...
an_lin_incr_test  date_test_rate_plateau  rate_testanc_inc  incr_test_rate_sympt  max_freq_testing  test_targeting  fx  gx
...
greater_disability_tox   greater_tox_zdv 
...
s_newp_this_per_art_or_prep   s_newp_this_per_art   s_newp_this_per_prep  s_newp_this_per
...
s_an_lin_incr_test  s_date_test_rate_plateau  s_rate_testanc_inc  s_incr_test_rate_sympt  s_max_freq_testing  s_test_targeting  s_fx  s_gx
...
s_greater_disability_tox   s_greater_tox_zdv 
...
incidence1549w = (s_primary1549w * 100 * 12) / (s_alive1549_w - s_hiv1549w + s_primary1549w);
incidence1549m = (s_primary1549m * 100 * 12) / (s_alive1549_m - s_hiv1549m + s_primary1549m);
*/
...
%update_r1(da1=1,da2=2,e=5,f=6,g=145,h=152,a=147,b=146,i=145,k=144,l=143,x=148,j=149,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=145,h=152,a=148,b=147,i=146,k=145,l=144,x=149,j=150,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=145,h=152,a=149,b=148,i=147,k=146,l=145,x=150,j=151,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=145,h=152,a=150,b=149,i=148,k=147,l=146,x=151,j=152,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=149,h=156,a=151,b=150,i=149,k=148,l=147,x=152,j=153,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=149,h=156,a=152,b=151,i=150,k=149,l=148,x=153,j=154,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=149,h=156,a=153,b=152,i=151,k=150,l=149,x=154,j=155,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=149,h=156,a=154,b=153,i=152,k=151,l=150,x=155,j=156,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=153,h=160,a=155,b=154,i=153,k=152,l=151,x=156,j=157,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=153,h=160,a=156,b=155,i=154,k=153,l=152,x=157,j=158,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=153,h=160,a=157,b=156,i=155,k=154,l=153,x=158,j=159,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=153,h=160,a=158,b=157,i=156,k=155,l=154,x=159,j=160,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=157,h=164,a=159,b=158,i=157,k=156,l=155,x=160,j=161,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=157,h=164,a=160,b=159,i=158,k=157,l=156,x=161,j=162,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=157,h=164,a=161,b=160,i=159,k=158,l=157,x=162,j=163,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=157,h=164,a=162,b=161,i=160,k=159,l=158,x=163,j=164,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=161,h=168,a=163,b=162,i=161,k=160,l=159,x=164,j=165,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=161,h=168,a=164,b=163,i=162,k=161,l=160,x=165,j=166,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=161,h=168,a=165,b=164,i=163,k=162,l=161,x=166,j=167,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=161,h=168,a=166,b=165,i=164,k=163,l=162,x=167,j=168,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=165,h=172,a=167,b=166,i=165,k=164,l=163,x=168,j=169,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=165,h=172,a=168,b=167,i=166,k=165,l=164,x=169,j=170,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=165,h=172,a=169,b=168,i=167,k=166,l=165,x=170,j=171,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=165,h=172,a=170,b=169,i=168,k=167,l=166,x=171,j=172,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=169,h=176,a=171,b=170,i=169,k=168,l=167,x=172,j=173,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=169,h=176,a=172,b=171,i=170,k=169,l=168,x=173,j=174,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=169,h=176,a=173,b=172,i=171,k=170,l=169,x=174,j=175,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=169,h=176,a=174,b=173,i=172,k=171,l=170,x=175,j=176,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=173,h=180,a=175,b=174,i=173,k=172,l=171,x=176,j=177,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=173,h=180,a=176,b=175,i=174,k=173,l=172,x=177,j=178,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=173,h=180,a=177,b=176,i=175,k=174,l=173,x=178,j=179,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=173,h=180,a=178,b=177,i=176,k=175,l=174,x=179,j=180,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=177,h=184,a=179,b=178,i=177,k=176,l=175,x=180,j=181,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=177,h=184,a=180,b=179,i=178,k=177,l=176,x=181,j=182,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=177,h=184,a=181,b=180,i=179,k=178,l=177,x=182,j=183,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=177,h=184,a=182,b=181,i=180,k=179,l=178,x=183,j=184,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=181,h=188,a=183,b=182,i=181,k=180,l=179,x=184,j=185,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=181,h=188,a=184,b=183,i=182,k=181,l=180,x=185,j=186,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=181,h=188,a=185,b=184,i=183,k=182,l=181,x=186,j=187,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=181,h=188,a=186,b=185,i=184,k=183,l=182,x=187,j=188,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=185,h=192,a=187,b=186,i=185,k=184,l=183,x=188,j=189,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=185,h=192,a=188,b=187,i=186,k=185,l=184,x=189,j=190,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=185,h=192,a=189,b=188,i=187,k=186,l=185,x=190,j=191,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=185,h=192,a=190,b=189,i=188,k=187,l=186,x=191,j=192,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=189,h=196,a=191,b=190,i=189,k=188,l=187,x=192,j=193,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=189,h=196,a=192,b=191,i=190,k=189,l=188,x=193,j=194,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=189,h=196,a=193,b=192,i=191,k=190,l=189,x=194,j=195,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=189,h=196,a=194,b=193,i=192,k=191,l=190,x=195,j=196,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=193,h=200,a=195,b=194,i=193,k=192,l=191,x=196,j=197,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=193,h=200,a=196,b=195,i=194,k=193,l=192,x=197,j=198,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=193,h=200,a=197,b=196,i=195,k=194,l=193,x=198,j=199,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=193,h=200,a=198,b=197,i=196,k=195,l=194,x=199,j=200,s=2);   
%update_r1(da1=1,da2=2,e=5,f=6,g=197,h=204,a=199,b=198,i=197,k=196,l=195,x=200,j=201,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=197,h=204,a=200,b=199,i=198,k=197,l=196,x=201,j=202,s=2);
%update_r1(da1=1,da2=2,e=7,f=8,g=197,h=204,a=201,b=200,i=199,k=198,l=197,x=202,j=203,s=2);
%update_r1(da1=2,da2=1,e=8,f=9,g=197,h=204,a=202,b=201,i=200,k=199,l=198,x=203,j=204,s=2);
%update_r1(da1=1,da2=2,e=5,f=6,g=201,h=208,a=203,b=202,i=201,k=200,l=199,x=204,j=205,s=2);
%update_r1(da1=2,da2=1,e=6,f=7,g=201,h=208,a=204,b=203,i=202,k=201,l=200,x=205,j=206,s=2);
...
%update_r1(da1=1,da2=2,e=5,f=6,g=145,h=152,a=147,b=146,i=145,k=144,l=143,x=148,j=149,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=145,h=152,a=148,b=147,i=146,k=145,l=144,x=149,j=150,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=145,h=152,a=149,b=148,i=147,k=146,l=145,x=150,j=151,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=145,h=152,a=150,b=149,i=148,k=147,l=146,x=151,j=152,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=149,h=156,a=151,b=150,i=149,k=148,l=147,x=152,j=153,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=149,h=156,a=152,b=151,i=150,k=149,l=148,x=153,j=154,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=149,h=156,a=153,b=152,i=151,k=150,l=149,x=154,j=155,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=149,h=156,a=154,b=153,i=152,k=151,l=150,x=155,j=156,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=153,h=160,a=155,b=154,i=153,k=152,l=151,x=156,j=157,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=153,h=160,a=156,b=155,i=154,k=153,l=152,x=157,j=158,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=153,h=160,a=157,b=156,i=155,k=154,l=153,x=158,j=159,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=153,h=160,a=158,b=157,i=156,k=155,l=154,x=159,j=160,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=157,h=164,a=159,b=158,i=157,k=156,l=155,x=160,j=161,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=157,h=164,a=160,b=159,i=158,k=157,l=156,x=161,j=162,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=157,h=164,a=161,b=160,i=159,k=158,l=157,x=162,j=163,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=157,h=164,a=162,b=161,i=160,k=159,l=158,x=163,j=164,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=161,h=168,a=163,b=162,i=161,k=160,l=159,x=164,j=165,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=161,h=168,a=164,b=163,i=162,k=161,l=160,x=165,j=166,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=161,h=168,a=165,b=164,i=163,k=162,l=161,x=166,j=167,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=161,h=168,a=166,b=165,i=164,k=163,l=162,x=167,j=168,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=165,h=172,a=167,b=166,i=165,k=164,l=163,x=168,j=169,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=165,h=172,a=168,b=167,i=166,k=165,l=164,x=169,j=170,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=165,h=172,a=169,b=168,i=167,k=166,l=165,x=170,j=171,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=165,h=172,a=170,b=169,i=168,k=167,l=166,x=171,j=172,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=169,h=176,a=171,b=170,i=169,k=168,l=167,x=172,j=173,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=169,h=176,a=172,b=171,i=170,k=169,l=168,x=173,j=174,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=169,h=176,a=173,b=172,i=171,k=170,l=169,x=174,j=175,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=169,h=176,a=174,b=173,i=172,k=171,l=170,x=175,j=176,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=173,h=180,a=175,b=174,i=173,k=172,l=171,x=176,j=177,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=173,h=180,a=176,b=175,i=174,k=173,l=172,x=177,j=178,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=173,h=180,a=177,b=176,i=175,k=174,l=173,x=178,j=179,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=173,h=180,a=178,b=177,i=176,k=175,l=174,x=179,j=180,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=177,h=184,a=179,b=178,i=177,k=176,l=175,x=180,j=181,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=177,h=184,a=180,b=179,i=178,k=177,l=176,x=181,j=182,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=177,h=184,a=181,b=180,i=179,k=178,l=177,x=182,j=183,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=177,h=184,a=182,b=181,i=180,k=179,l=178,x=183,j=184,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=181,h=188,a=183,b=182,i=181,k=180,l=179,x=184,j=185,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=181,h=188,a=184,b=183,i=182,k=181,l=180,x=185,j=186,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=181,h=188,a=185,b=184,i=183,k=182,l=181,x=186,j=187,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=181,h=188,a=186,b=185,i=184,k=183,l=182,x=187,j=188,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=185,h=192,a=187,b=186,i=185,k=184,l=183,x=188,j=189,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=185,h=192,a=188,b=187,i=186,k=185,l=184,x=189,j=190,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=185,h=192,a=189,b=188,i=187,k=186,l=185,x=190,j=191,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=185,h=192,a=190,b=189,i=188,k=187,l=186,x=191,j=192,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=189,h=196,a=191,b=190,i=189,k=188,l=187,x=192,j=193,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=189,h=196,a=192,b=191,i=190,k=189,l=188,x=193,j=194,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=189,h=196,a=193,b=192,i=191,k=190,l=189,x=194,j=195,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=189,h=196,a=194,b=193,i=192,k=191,l=190,x=195,j=196,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=193,h=200,a=195,b=194,i=193,k=192,l=191,x=196,j=197,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=193,h=200,a=196,b=195,i=194,k=193,l=192,x=197,j=198,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=193,h=200,a=197,b=196,i=195,k=194,l=193,x=198,j=199,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=193,h=200,a=198,b=197,i=196,k=195,l=194,x=199,j=200,s=3);   
%update_r1(da1=1,da2=2,e=5,f=6,g=197,h=204,a=199,b=198,i=197,k=196,l=195,x=200,j=201,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=197,h=204,a=200,b=199,i=198,k=197,l=196,x=201,j=202,s=3);
%update_r1(da1=1,da2=2,e=7,f=8,g=197,h=204,a=201,b=200,i=199,k=198,l=197,x=202,j=203,s=3);
%update_r1(da1=2,da2=1,e=8,f=9,g=197,h=204,a=202,b=201,i=200,k=199,l=198,x=203,j=204,s=3);
%update_r1(da1=1,da2=2,e=5,f=6,g=201,h=208,a=203,b=202,i=201,k=200,l=199,x=204,j=205,s=3);
%update_r1(da1=2,da2=1,e=6,f=7,g=201,h=208,a=204,b=203,i=202,k=201,l=200,x=205,j=206,s=3);



data r1; set a;

%update_r1(da1=1,da2=2,e=7,f=8,g=121,h=128,a=125,b=124,i=123,k=122,l=121,x=126,j=127,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=121,h=128,a=126,b=125,i=124,k=123,l=122,x=127,j=128,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=125,h=132,a=127,b=126,i=125,k=124,l=123,x=128,j=129,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=125,h=132,a=128,b=127,i=126,k=125,l=124,x=129,j=130,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=125,h=132,a=129,b=128,i=127,k=126,l=125,x=130,j=131,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=125,h=132,a=130,b=129,i=128,k=127,l=126,x=131,j=132,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=129,h=136,a=131,b=130,i=129,k=128,l=127,x=132,j=133,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=129,h=136,a=132,b=131,i=130,k=129,l=128,x=133,j=134,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=129,h=136,a=133,b=132,i=131,k=130,l=129,x=134,j=135,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=129,h=136,a=134,b=133,i=132,k=131,l=130,x=135,j=136,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=133,h=140,a=135,b=134,i=133,k=132,l=131,x=136,j=137,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=133,h=140,a=136,b=135,i=134,k=133,l=132,x=137,j=138,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=133,h=140,a=137,b=136,i=135,k=134,l=133,x=138,j=139,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=133,h=140,a=138,b=137,i=136,k=135,l=134,x=139,j=140,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=137,h=144,a=139,b=138,i=137,k=136,l=135,x=140,j=141,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=137,h=144,a=140,b=139,i=138,k=137,l=136,x=141,j=142,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=137,h=144,a=141,b=140,i=139,k=138,l=137,x=142,j=143,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=137,h=144,a=142,b=141,i=140,k=139,l=138,x=143,j=144,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=141,h=148,a=143,b=142,i=141,k=140,l=139,x=144,j=145,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=141,h=148,a=144,b=143,i=142,k=141,l=140,x=145,j=146,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=141,h=148,a=145,b=144,i=143,k=142,l=141,x=146,j=147,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=141,h=148,a=146,b=145,i=144,k=143,l=142,x=147,j=148,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=145,h=152,a=147,b=146,i=145,k=144,l=143,x=148,j=149,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=145,h=152,a=148,b=147,i=146,k=145,l=144,x=149,j=150,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=145,h=152,a=149,b=148,i=147,k=146,l=145,x=150,j=151,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=145,h=152,a=150,b=149,i=148,k=147,l=146,x=151,j=152,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=149,h=156,a=151,b=150,i=149,k=148,l=147,x=152,j=153,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=149,h=156,a=152,b=151,i=150,k=149,l=148,x=153,j=154,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=149,h=156,a=153,b=152,i=151,k=150,l=149,x=154,j=155,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=149,h=156,a=154,b=153,i=152,k=151,l=150,x=155,j=156,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=153,h=160,a=155,b=154,i=153,k=152,l=151,x=156,j=157,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=153,h=160,a=156,b=155,i=154,k=153,l=152,x=157,j=158,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=153,h=160,a=157,b=156,i=155,k=154,l=153,x=158,j=159,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=153,h=160,a=158,b=157,i=156,k=155,l=154,x=159,j=160,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=157,h=164,a=159,b=158,i=157,k=156,l=155,x=160,j=161,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=157,h=164,a=160,b=159,i=158,k=157,l=156,x=161,j=162,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=157,h=164,a=161,b=160,i=159,k=158,l=157,x=162,j=163,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=157,h=164,a=162,b=161,i=160,k=159,l=158,x=163,j=164,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=161,h=168,a=163,b=162,i=161,k=160,l=159,x=164,j=165,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=161,h=168,a=164,b=163,i=162,k=161,l=160,x=165,j=166,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=161,h=168,a=165,b=164,i=163,k=162,l=161,x=166,j=167,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=161,h=168,a=166,b=165,i=164,k=163,l=162,x=167,j=168,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=165,h=172,a=167,b=166,i=165,k=164,l=163,x=168,j=169,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=165,h=172,a=168,b=167,i=166,k=165,l=164,x=169,j=170,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=165,h=172,a=169,b=168,i=167,k=166,l=165,x=170,j=171,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=165,h=172,a=170,b=169,i=168,k=167,l=166,x=171,j=172,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=169,h=176,a=171,b=170,i=169,k=168,l=167,x=172,j=173,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=169,h=176,a=172,b=171,i=170,k=169,l=168,x=173,j=174,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=169,h=176,a=173,b=172,i=171,k=170,l=169,x=174,j=175,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=169,h=176,a=174,b=173,i=172,k=171,l=170,x=175,j=176,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=173,h=180,a=175,b=174,i=173,k=172,l=171,x=176,j=177,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=173,h=180,a=176,b=175,i=174,k=173,l=172,x=177,j=178,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=173,h=180,a=177,b=176,i=175,k=174,l=173,x=178,j=179,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=173,h=180,a=178,b=177,i=176,k=175,l=174,x=179,j=180,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=177,h=184,a=179,b=178,i=177,k=176,l=175,x=180,j=181,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=177,h=184,a=180,b=179,i=178,k=177,l=176,x=181,j=182,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=177,h=184,a=181,b=180,i=179,k=178,l=177,x=182,j=183,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=177,h=184,a=182,b=181,i=180,k=179,l=178,x=183,j=184,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=181,h=188,a=183,b=182,i=181,k=180,l=179,x=184,j=185,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=181,h=188,a=184,b=183,i=182,k=181,l=180,x=185,j=186,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=181,h=188,a=185,b=184,i=183,k=182,l=181,x=186,j=187,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=181,h=188,a=186,b=185,i=184,k=183,l=182,x=187,j=188,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=185,h=192,a=187,b=186,i=185,k=184,l=183,x=188,j=189,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=185,h=192,a=188,b=187,i=186,k=185,l=184,x=189,j=190,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=185,h=192,a=189,b=188,i=187,k=186,l=185,x=190,j=191,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=185,h=192,a=190,b=189,i=188,k=187,l=186,x=191,j=192,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=189,h=196,a=191,b=190,i=189,k=188,l=187,x=192,j=193,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=189,h=196,a=192,b=191,i=190,k=189,l=188,x=193,j=194,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=189,h=196,a=193,b=192,i=191,k=190,l=189,x=194,j=195,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=189,h=196,a=194,b=193,i=192,k=191,l=190,x=195,j=196,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=193,h=200,a=195,b=194,i=193,k=192,l=191,x=196,j=197,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=193,h=200,a=196,b=195,i=194,k=193,l=192,x=197,j=198,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=193,h=200,a=197,b=196,i=195,k=194,l=193,x=198,j=199,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=193,h=200,a=198,b=197,i=196,k=195,l=194,x=199,j=200,s=4);   
%update_r1(da1=1,da2=2,e=5,f=6,g=197,h=204,a=199,b=198,i=197,k=196,l=195,x=200,j=201,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=197,h=204,a=200,b=199,i=198,k=197,l=196,x=201,j=202,s=4);
%update_r1(da1=1,da2=2,e=7,f=8,g=197,h=204,a=201,b=200,i=199,k=198,l=197,x=202,j=203,s=4);
%update_r1(da1=2,da2=1,e=8,f=9,g=197,h=204,a=202,b=201,i=200,k=199,l=198,x=203,j=204,s=4);
%update_r1(da1=1,da2=2,e=5,f=6,g=201,h=208,a=203,b=202,i=201,k=200,l=199,x=204,j=205,s=4);
%update_r1(da1=2,da2=1,e=6,f=7,g=201,h=208,a=204,b=203,i=202,k=201,l=200,x=205,j=206,s=4);
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_dolswitch_21_12_19_9am_&dataset_id";  
put   
