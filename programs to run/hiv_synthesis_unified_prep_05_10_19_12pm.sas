...
if 15 <= age1 < 18 and life_sex_risk ge 2 and e < 0.060 then sw1=1;
...
* pop_wide_tld_2020;	pop_wide_tld_2020 = 0;
						if _u47 < 0.40 then do;  
							...
							decr_rate_choose_stop_prep_2020 = 1; eff_rate_choose_stop_prep = 0.05 ; 
...
* decr_r_choose_stop_prep_2020;
						decr_rate_choose_stop_prep_2020 = 0;  
						if _u30 < 0.95 then do; 
							decr_rate_choose_stop_prep_2020 = 1; 
...
if t ge 2 and newp{t-1} = 0 then do;
	...
	if 0.6 <= e < 0.9 then do; q=uniform(0); 
...
if  t ge 2 and 1 <= newp{t-1} <= 6 then do;
	...
	if 0.1 <= e < 0.9 then do; q=uniform(0); 
		if q < 0.7 then newp{t}=1; if 0.7 <= q < 0.8 then newp{t}=2; if 0.8 <= q < 0.9 then newp{t}=3; if 0.9 <= q < 0.95 then newp{t}=4;    
		if 0.95 <= q < 0.98 then newp{t}=5; if 0.98 <= q       then newp{t}=6;    
	end;
...
if  t ge 2 and 7 <= newp{t-1} <= 40 then do;
	...
	if 0.9 <= e < 0.95 then do; q=uniform(0); 	newp{t} = 41 + (q*89); newp{t} = round(newp{t},1);  end;
	if 0.95 <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;

if  t ge 2 and  41 <= newp{t-1} <= 130 then do;
	if age{t} > 30 then e=e*0.9; * older women cant be in highest category ;
	if e < 0.05 then newp{t}=0;
	if 0.05 <= e < 0.1 then do; q=uniform(0); 
		if q < 0.7 then newp{t}=1; if 0.7 <= q < 0.8 then newp{t}=2; if 0.8 <= q < 0.9 then newp{t}=3; if 0.9 <= q < 0.95 then newp{t}=4;    
		if 0.95 <= q < 0.98 then newp{t}=5; if 0.98 <= q       then newp{t}=6;    
	...
	if 0.6 <= e < 0.9  then do; q=uniform(0); 	newp{t} = 41 + (q*89); newp{t} = round(newp{t},1);  end;
	if 0.9  <= e       then do; q=uniform(0); 	newp{t} = 131 + (q*50 ); newp{t} = round(newp{t},1);  end;
end;
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_prep_05_10_19_12pm_&dataset_id";  
put   
