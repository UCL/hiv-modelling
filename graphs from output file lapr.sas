...
prop_cab_res_o_cab    prop_cab_res_tail     prop_cab_res_1st_per     prop_prep_inj_at_inf_diag     of_all_o_cab_prop_dur_3m     of_all_o_cab_prop_dur_6m     p_prep_inj_hiv  p_prep_adhg80_  n_cur_res_cab  n_cur_res_dol
...
n_prep_all_start   n_prep_oral_start     n_prep_inj_start    prop_cab_dol_res_attr_cab  p_emerge_inm_res_cab_tail  p_emerge_inm_res_cab
...
of_all_o_cab_prop_dur_3m     of_all_o_cab_prop_dur_6m     p_prep_inj_hiv  p_prep_adhg80_  n_cur_res_cab  n_cur_res_dol
n_o_cab_at_3m   n_o_cab_at_6m   n_o_cab_at_9m   n_o_cab_at_ge12m
n_emerge_inm_res_cab  n_switch_prep_from_oral  n_switch_prep_from_inj  n_switch_prep_to_oral  n_switch_prep_to_inj  
n_prep_all_start n_prep_oral_start n_prep_inj_start prop_cab_dol_res_attr_cab  p_emerge_inm_res_cab_tail  p_emerge_inm_res_cab
...
ods html;
proc sgplot data=d; 
Title    height=1.5 justify=center "Amongst people with (undiagnosed) HIV and on cab-la, number in whom integrase inhibitor resistance has arisen in this period";
xaxis label			= 'Year'		labelattrs=(size=12)  values = (1993 to &year_end by 2)	 	 valueattrs=(size=10); 
yaxis grid label	= 'Proportion'		labelattrs=(size=12)  values = (0 to 100000 by 10000) valueattrs=(size=10);

label p50_n_emerge_inm_res_cab_0 = "no cab-la introduction (median) ";
label p50_n_emerge_inm_res_cab_1 = "cab-la introduction (median) ";

series  x=cald y=p50_n_emerge_inm_res_cab_0/	lineattrs = (color=black thickness = 2);
band    x=cald lower=p5_n_emerge_inm_res_cab_0 	upper=p95_n_emerge_inm_res_cab_0  / transparency=0.9 fillattrs = (color=black) legendlabel= "90% range";
series  x=cald y=p50_n_emerge_inm_res_cab_1/	lineattrs = (color=str thickness = 2);
band    x=cald lower=p5_n_emerge_inm_res_cab_1 	upper=p95_n_emerge_inm_res_cab_1  / transparency=0.9 fillattrs = (color=str) legendlabel= "90% range";
...
ods html;
proc sgplot data=d; 
Title    height=1.5 justify=center "Of all people on cab-la, proportion who have been infected with HIV 3-6 months ago";
xaxis label			= 'Year'		labelattrs=(size=12)  values = (1993 to &year_end by 2)	 	 valueattrs=(size=10); 
yaxis grid label	= 'Proportion'		labelattrs=(size=12)  values = (0 to 1     by 0.1 ) valueattrs=(size=10);
