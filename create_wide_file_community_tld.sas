...
libname b "C:\Users\w3sth\Dropbox (UCL)\hiv synthesis ssa unified program\output files\pop_wide_tld\pop_wide_tld_revision_out\";

data i; set b.out:; 

data pwt_rev_a  ;  set i; 
...
keep run sf_2024 incidence1549_2022 prevalence1549_2022;
...
dead_ddaly_odabe = s_dead_ddaly_oth_dol_adv_birth_e * &sf * 4; 

ddaly_mtct = s_ddaly_mtct * &sf * 4;

ddaly_non_aids_pre_death = s_ddaly_non_aids_pre_death * &sf * 4; 
...
* p_newp_this_per_prep;			p_newp_this_per_prep = s_newp_this_per_prep / s_newp_this_per_hivneg ;  
...
* p_newp_prep;					p_newp_prep = s_prep_newp / (s_m_newp + s_w_newp) ;  

* p_newp_prep_hivneg;			p_newp_prep_hivneg = s_prep_newp / s_newp_hivneg ;  
...
* n_pop_wide_tld_hiv ;			n_pop_wide_tld_hiv = s_pop_wide_tld_hiv * &sf ;
...
* p_artexp_onart; 				if s_artexp > 0 then p_artexp_onart = s_onart / s_artexp;
...
* prev_vg1000_1549 ;			prev_vg1000_1549 = s_vg1000_1549 / s_alive1549;
...
n_preg_odabe = s_pregnant_oth_dol_adv_birth_e * &sf * 4;  
...
prevalence_vg1000 incidence1549  incidence1564  prevalence1524m prevalence1524w prevalence_sw incidence1549w  incidence1549m  incidence_sw incidence_onprep
...
p_ai_no_arv_e_inm p_artexp_diag p_artexp_onart p_onart_diag p_onart_diag_w p_onart_diag_m p_onart_diag_sw p_efa p_taz
...
ddaly daly  p_emerge_inm_res_cab  p_emerge_inm_res_cab_tail of_all_o_cab_prop_dur_9m of_all_o_cab_prop_dur_ge12m
...
s_pop_wide_tld_prep_elig  s_pop_wide_tld_prep  n_pop_wide_tld_as_art
...
low_prep_inj_uptake pop_wide_tld_selective_hiv  death_r_iris_pop_wide_tld  rr_mort_tdf_prep  prob_test_pop_wide_tld_prep n_hiv1_prep_oral
;
...
proc sort data= a.pwt_rev_b out= a.pwt_rev_c ; by run option; run;

proc freq data=a.pwt_rev_c ; tables run; where cald = 2022; run;


%let sf=sf_2024;

data    a.l_pwt_rev       ; set a.pwt_rev_c;  
...
proc means  noprint data=e; var &v; output out=y_24 mean= &v._24; by run ; where 2023.5 <= cald < 2024.5; 

proc means noprint data=e; var &v; output out=y_20y mean= &v._20y; by run option ; where 2024.5 <= cald < 2044.50;   

proc means noprint data=e; var &v; output out=y_3y mean= &v._3y; by run option ; where 2024.5 <= cald < 2027.50;   
proc means noprint data=e; var &v; output out=y_5y mean= &v._5y; by run option ; where 2024.5 <= cald < 2029.50;   

proc means noprint data=e; var &v; output out=y_50y mean= &v._50y; by run option ; where 2024.5 <= cald < 2074.50;

proc means noprint data=e; var &v; output out=y_29 mean= &v._29; by run option ; where 2028.5 <= cald < 2029.50;
...
data &v ; merge y_22 y_24 t_20y  t_3y  t_5y t_29  t_44 t_50y ;  
...
%var(v=p_mcirc) ;  %var(v=p_mcirc_1549m);
...
%var(v=p_artexp_diag); %var(v=p_artexp_onart); 
%var(v=p_onart_diag); %var(v=p_onart_diag_w); %var(v=p_onart_diag_m); %var(v=p_onart_diag_sw); %var(v=p_efa); %var(v=p_taz);
...
%var(v=p_onart_vl1000_w); %var(v=p_onart_vl1000_m);  %var(v= p_startedline2) ; %var(v=p_linefail_ge1);
 %var(v=death_rate_onart);     %var(v=dcost);  %var(v=cost);    %var(v= dart_cost_y);
...
%var(v=p_onart_cd4_l500);  %var(v=prop_art_or_prep);    %var(v=prop_sw_onprep);   %var(v=p_onart);
    %var(v=p_adh_hi);   %var(v=p_nactive_ge2p00_xyz);   %var(v=p_nactive_ge1p50_xyz); 
   %var(v=p_k65m);   %var(v=p_m184m);
...
%var(v=p_iime); %var(v=n_infected_inm);   %var(v=n_birth_with_inf_child);  %var(v=n_tested); %var(v=n_tested_sw); %var(v=test_prop_positive);
%var(v=p_vlg1000_onart_65m);   %var(v=p_vlg1000_onart_184m);   %var(v=p_elig_prep); %var(v=prop_elig_on_prep);  
 %var(v= n_hiv1_prep_oral);
  %var(v= n_prep_any);   %var(v=n_death_hiv);   %var(v= n_hiv);
%var(v=p_prep_any_ever); %var(v=p_hiv1_prep);  %var(v=p_hiv1_prep_inj); %var(v=p_hiv1_prep_oral); %var(v=incidence1524w);  
 %var(v=incidence_sw);   %var(v=n_dead_hivpos_cause1 ); %var(v=rate_dead_hivpos_cause1);  %var(v=n_cd4_lt200);
 %var(v=rate_dead_hivpos_anycause); 
%var(v=n_onart);
%var(v=p_onart_w); %var(v=p_onart_m);   %var(v=p_diag_w); %var(v=p_diag_m); 
...
%var(v=of_all_o_cab_prop_dur_3m); %var(v=n_pop_wide_tld_as_art);
...
%var(v=ddaly); %var(v=daly); %var(v=p_emerge_inm_res_cab);  %var(v=p_emerge_inm_res_cab_tail); %var(v=ddaly_ac_ntd_mtct);
...
%var(v=p_pop_tld_neg_prep_inel); %var(v=n_pop_wide_tld_hiv);   %var(v=n_pop_wide_tld_prep_elig);   
...
n_cd4_lt200 n_inf_pre_yr_interv  n_inf_post_yr_interv n_death_hiv_inf_pre_yr_interv   n_death_hiv_inf_post_yr_interv n_hiv1_prep_oral
;
...
* &p ;
proc means noprint data=f; var &p ; output out=y_ mean= &p; by run ; where cald = 2024.5 and option=1; run;
...
%par(p=rr_return_pop_wide_tld); %par(p=rr_interrupt_pop_wide_tld);  %par(p=prob_tld_hiv_concern);  %par(p=prob_onartvis0_0_to_1);
...
%par(p=inc_oral_prep_pref_pop_wide_tld);
 ...
 %par(p= prep_vlg1000_threshold);    %par(p=prop_pep); %par(p=pep_efficacy);  %par(p=artvis0_lower_adh);  %par(p=pop_wide_prep_adh_effect);
 %par(p=low_prep_inj_uptake); %par(p=pop_wide_tld_selective_hiv); %par(p=death_r_iris_pop_wide_tld); %par(p=rr_mort_tdf_prep);
...
p_emerge_inm_res_cab_notpr res_level_dol_cab_mut  pr_res_dol prep_dependent_prev_vg1000 
 prep_vlg1000_threshold   prop_pep pep_efficacy artvis0_lower_adh pop_wide_prep_adh_effect  low_prep_inj_uptake  pop_wide_tld_selective_hiv
death_r_iris_pop_wide_tld  rr_mort_tdf_prep  prob_test_pop_wide_tld_prep
;
...
dcost_50y_1 = dart_cost_y_50y_1 + dadc_cost_50y_1 + dcd4_cost_50y_1 + dvl_cost_50y_1 + dvis_cost_50y_1 + dnon_tb_who3_cost_50y_1 + 
					...
					+ (dcost_prep_visit_oral_50y_1) + (dcost_prep_oral_50y_1) + (dcost_prep_visit_inj_50y_1) + (dcost_prep_inj_50y_1)
;			
...
dcost_prep_total_50y_1 = (dcost_prep_visit_oral_50y_1) + (dcost_prep_oral_50y_1)+ (dcost_prep_visit_inj_50y_1) + (dcost_prep_inj_50y_1);
...
pop_wide_tld_ce=0; if netdaly500_2 = min_netdaly500 then pop_wide_tld_ce=1;
pop_wide_tld_ce_x = 1 - pop_wide_tld_ce;

d_n_death_hiv_20y_2_1 = n_death_hiv_20y_2 - n_death_hiv_20y_1;
...
d_p_onart_20y_2_1 = p_onart_20y_2 - p_onart_20y_1 ;
d_p_elig_hivneg_onprep_20y_2_1 =   p_elig_hivneg_onprep_20y_2 -   p_elig_hivneg_onprep_20y_1 ;

d_prop_elig_on_prep_3y_2_1 = prop_elig_on_prep_3y_2 - prop_elig_on_prep_3y_1;
d_prop_1564_onprep_3y_2_1 = prop_1564_onprep_3y_2 - prop_1564_onprep_3y_1;
d_prop_prep_inj_3y_2_1 = prop_prep_inj_3y_2 - prop_prep_inj_3y_1 ;
d_p_onart_3y_2_1 = p_onart_3y_2 - p_onart_3y_1 ;
d_p_elig_hivneg_onprep_3y_2_1 =   p_elig_hivneg_onprep_3y_2 -   p_elig_hivneg_onprep_3y_1 ;
d_p_onart_m_3y_2_1 = p_onart_m_3y_2 - p_onart_m_3y_1 ;
d_p_onart_w_3y_2_1 = p_onart_w_3y_2 - p_onart_w_3y_1 ;
d_prevalence_vg1000_3y_2_1 = prevalence_vg1000_3y_2 - prevalence_vg1000_3y_1;
d_p_oral_pep_not_prep_3y_2_1 = p_oral_pep_not_prep_3y_2 - p_oral_pep_not_prep_3y_1 ;
d_p_onart_vl1000_3y_2_1 = p_onart_vl1000_3y_2 - p_onart_vl1000_3y_1 ; 
d_p_vl1000_3y_2_1 = p_vl1000_3y_2 - p_vl1000_3y_1 ; 

d_prop_elig_on_prep_5y_2_1 = prop_elig_on_prep_5y_2 - prop_elig_on_prep_5y_1;
d_prop_1564_onprep_5y_2_1 = prop_1564_onprep_5y_2 - prop_1564_onprep_5y_1;
d_prop_prep_inj_5y_2_1 = prop_prep_inj_5y_2 - prop_prep_inj_5y_1 ;
d_p_onart_5y_2_1 = p_onart_5y_2 - p_onart_5y_1 ;
d_p_elig_hivneg_onprep_5y_2_1 =   p_elig_hivneg_onprep_5y_2 -   p_elig_hivneg_onprep_5y_1 ;
...
d_incidence_onprep_20y_2_1 = incidence_onprep_20y_2 - incidence_onprep_20y_1 ;
d_n_birth_with_inf_child_20y_2_1 = n_birth_with_inf_child_20y_2 - n_birth_with_inf_child_20y_1 ;
r_prevalence1549_44_2_1 = prevalence1549_44_2 / prevalence1549_44_1 ;
r_prevalence1549_50y_2_1 = prevalence1549_50y_2 / prevalence1549_50y_1 ;
r_n_hiv_44_2_1 = n_hiv_44_2 / n_hiv_44_1 ;
...
d_n_st_rest_prep_inj_hiv_20y_2_1 = n_start_rest_prep_inj_hiv_20y_2 - n_start_rest_prep_inj_hiv_20y_1 ; 
d_n_prep_inj_20y_2_1 = n_prep_inj_20y_2 - n_prep_inj_20y_1 ;
d_p_ai_no_arv_e_inm_44_2_1 = p_ai_no_arv_e_inm_44_2 - p_ai_no_arv_e_inm_44_1 ;
d_p_iime_44_2_1 = p_iime_44_2 - p_iime_44_1 ;
d_p_nrtime_all_adults_44_2_1 = p_nrtime_all_adults_44_2 - p_nrtime_all_adults_44_1 ;
d_n_infected_inm_44_2_1 = n_infected_inm_44_2 - n_infected_inm_44_1 ;   
d_n_cur_res_cab_44_2_1 = n_cur_res_cab_44_2 - n_cur_res_cab_44_1 ;
d_p_vl1000_art_12m_onart_44_2_1 = p_vl1000_art_12m_onart_44_2 - p_vl1000_art_12m_onart_44_1 ;  
d_p_onart_vl1000_44_2_1 = p_onart_vl1000_44_2 - p_onart_vl1000_44_1 ; 
d_p_taz_44_2_1 = p_taz_44_2 - p_taz_44_1 ; 
d_p_iime_all_adults_44_2_1 = p_iime_all_adults_44_2 - p_iime_all_adults_44_1;
d_prevalence_vg1000_44_2_1 = prevalence_vg1000_44_2 - prevalence_vg1000_44_1;
d_incidence1549_20y_2_1 = incidence1549_20y_2 - incidence1549_20y_1 ;
d_p_vl1000_44_2_1 = p_vl1000_44_2 - p_vl1000_44_1 ; 
...
d_p_nacti_art_start_lt1p5_44_2_1 = p_nactive_art_start_lt1p5_44_2 - p_nactive_art_start_lt1p5_44_1 ;
d_p_nacti_art_start_lt2_44_2_1 = p_nactive_art_start_lt2_44_2 - p_nactive_art_start_lt2_44_1 ;
d_p_nacti_art_start_lt3_44_2_1 = p_nactive_art_start_lt3_44_2 - p_nactive_art_start_lt3_44_1 ;
...
d_p_ai_no_arv_e_inm_50y_2_1 = p_ai_no_arv_e_inm_50y_2 - p_ai_no_arv_e_inm_50y_1;
...
ods html;
proc means data=   x n p50 p5 p95 min max;  
...
ods html;
proc means  data=   x n median p5 p95 min max ;
...
d_p_onart_m_3y_2_1 p_onart_m_3y_1  p_onart_m_3y_2
d_p_onart_w_3y_2_1 p_onart_w_3y_1  p_onart_w_3y_2
p_onart_vl1000_3y_1  p_onart_vl1000_3y_2   d_p_onart_vl1000_3y_2_1 
d_p_vl1000_3y_2_1   p_vl1000_3y_2   p_vl1000_3y_1 
d_p_oral_pep_not_prep_3y_2_1   p_oral_pep_not_prep_3y_2  p_oral_pep_not_prep_3y_1 
p_onartvisit0_3y_2 
p_onartvisit0_vl1000_3y_2 
p_pop_tld_neg_prep_inel_3y_2  
...
p_onart_vl1000_44_1  p_onart_vl1000_44_2   d_p_onart_vl1000_44_2_1 
d_p_vl1000_44_2_1   p_vl1000_44_2   p_vl1000_44_1 
prop_prep_inj_20y_2 prop_prep_inj_20y_1  d_prop_prep_inj_20y_2_1
d_p_oral_pep_not_prep_20y_2_1   p_oral_pep_not_prep_20y_2  p_oral_pep_not_prep_20y_1 
p_elig_hivneg_onprep_44_1 p_elig_hivneg_onprep_44_2 
p_onartvisit0_44_2
p_onartvisit0_5y_2
p_onartvisit0_20y_2
p_onartvisit0_50y_2
p_onartvisit0_vl1000_44_2 
...
prop_prep_tot5yrs_44_1 prop_prep_tot5yrs_44_2 
p_prep_any_ever_44_1  p_prep_any_ever_44_2  d_p_prep_any_ever_44_2_1
incidence1549_20y_1 incidence1549_20y_2 r_incidence1549_20y_2_1
incidence1549_50y_1 incidence1549_50y_2 r_incidence1549_50y_2_1
incidence_onprep_20y_1 incidence_onprep_20y_2  d_incidence_onprep_20y_2_1
n_birth_with_inf_child_20y_1 n_birth_with_inf_child_20y_2  d_n_birth_with_inf_child_20y_2_1
prevalence1549_44_1 prevalence1549_44_2  r_prevalence1549_44_2_1
prevalence1549_50y_1 prevalence1549_50y_2  r_prevalence1549_50y_2_1
n_hiv_44_1 n_hiv_44_2 r_n_hiv_44_2_1 
...
p_ai_no_arv_e_inm_44_1 p_ai_no_arv_e_inm_44_2   d_p_ai_no_arv_e_inm_44_2_1
p_iime_44_1 p_iime_44_2  d_p_iime_44_2_1
n_infected_inm_44_1  n_infected_inm_44_2   d_n_infected_inm_44_2_1  
p_vl1000_art_12m_onart_44_1 p_vl1000_art_12m_onart_44_2   d_p_vl1000_art_12m_onart_44_2_1 
p_onart_vl1000_44_1  p_onart_vl1000_44_2   d_p_onart_vl1000_44_2_1 
prevalence_vg1000_44_1 prevalence_vg1000_44_2   d_prevalence_vg1000_44_2_1

p_vl1000_44_1  p_vl1000_44_2 
p_taz_44_1 p_taz_44_2 d_p_taz_44_2_1  
d_p_iime_all_adults_44_2_1   p_iime_all_adults_44_2   p_iime_all_adults_44_1 
d_p_nrtime_all_adults_44_2_1   p_nrtime_all_adults_44_2   p_nrtime_all_adults_44_1 
d_p_nacti_art_start_lt1p5_44_2_1  p_nactive_art_start_lt1p5_44_1  p_nactive_art_start_lt1p5_44_2 
d_p_nacti_art_start_lt2_44_2_1  p_nactive_art_start_lt2_44_1  p_nactive_art_start_lt2_44_2 
d_p_nacti_art_start_lt3_44_2_1  p_nactive_art_start_lt3_44_1  p_nactive_art_start_lt3_44_2 
...
ddaly_50y_1 ddaly_50y_2 d_ddaly_50y_2_1
 ...
 dart_cost_y_50y_1  dadc_cost_50y_1  dcd4_cost_50y_1  dvl_cost_50y_1  dvis_cost_50y_1  dnon_tb_who3_cost_50y_1  
					dcot_cost_50y_1  dtb_cost_50y_1  dres_cost_50y_1  dtest_cost_50y_1  d_t_adh_int_cost_50y_1  dswitchline_cost_50y_1  
					dcost_circ_50y_1  dcost_condom_dn_50y_1  dcost_child_hiv_50y_1  dcost_non_aids_pre_death_50y_1
					 dcost_prep_visit_oral_50y_1  dcost_prep_oral_50y_1  dcost_prep_visit_inj_50y_1  dcost_prep_inj_50y_1
...
if 0.3 <= d_p_elig_hivneg_onprep_3y_2_1       then d_p_elig_hivneg_onprep_3y_2_1_g=1;

if d_prop_prep_inj_3y_2_1 >= -0.1 then d_prop_prep_inj_3y_2_1_g=4;
if -0.1 > d_prop_prep_inj_3y_2_1 >= -0.2 then d_prop_prep_inj_3y_2_1_g=3;
if -0.2 > d_prop_prep_inj_3y_2_1 >= -0.3 then d_prop_prep_inj_3y_2_1_g=2;
if -0.3 > d_prop_prep_inj_3y_2_1         then d_prop_prep_inj_3y_2_1_g=1;

if d_prevalence_vg1000_3y_2_1 >= -0.003 then d_prevalence_vg1000_3y_2_1_g=3;
if -0.003 > d_prevalence_vg1000_3y_2_1 >= -0.006 then d_prevalence_vg1000_3y_2_1_g=2;
if -0.006 > d_prevalence_vg1000_3y_2_1           then d_prevalence_vg1000_3y_2_1_g=1;

if p_onartvisit0_3y_2 < 0.1 then p_onartvisit0_3y_2_g = 1;
if 0.1 <= p_onartvisit0_3y_2 < 0.2 then p_onartvisit0_3y_2_g = 2;
if 0.2 <= p_onartvisit0_3y_2 < 0.3 then p_onartvisit0_3y_2_g = 3;
if 0.3 <= p_onartvisit0_3y_2  then p_onartvisit0_3y_2_g = 4;

proc freq; tables
d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g;
run;

proc logistic ;  
class d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g;
model d_incidence1549_20y_2_1_gtp3_x   = d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g;
run;

proc   logistic  ; 
class d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g;
model d_incidence1549_20y_2_1_gtp3_x   = d_p_elig_hivneg_onprep_3y_2_1    d_prop_prep_inj_3y_2_1     d_prevalence_vg1000_3y_2_1    p_onartvisit0_3y_2    ;
run;
...
proc   logistic  ; 
class d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g;
model dalys_averted_x   = d_p_elig_hivneg_onprep_3y_2_1_g  d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g  ;
run;

proc   logistic  ; 
class d_p_elig_hivneg_onprep_3y_2_1_g d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g ;
model dalys_averted_x   =  d_p_elig_hivneg_onprep_3y_2_1  d_prop_prep_inj_3y_2_1    d_prevalence_vg1000_3y_2_1     p_onartvisit0_3y_2    ;
run;
...
proc logistic  ; 
class d_p_elig_hivneg_onprep_3y_2_1_g d_prop_prep_inj_3y_2_1_g   d_prevalence_vg1000_3y_2_1_g  p_onartvisit0_3y_2_g ;
model deaths_averted_x   =  d_p_elig_hivneg_onprep_3y_2_1  d_prop_prep_inj_3y_2_1    d_prevalence_vg1000_3y_2_1     p_onartvisit0_3y_2    ;
run;
...
class 
prep_dependent_prev_vg1000_g ;
model deaths_averted = 
pref_prep_oral_beta_s1 
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld  prob_tld_hiv_concern  prob_onartvis0_0_to_1 prob_onartvis0_1_to_0 rate_choose_stop_prep_oral
...
greater_disability_tox prob_tld_hiv_concern p_hard_reach_m p_hard_reach_w;
run;
...
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld    prob_onartvis0_0_to_1 prob_onartvis0_1_to_0
prob_prep_pop_wide_tld  inc_oral_prep_pref_pop_wide_tld  prep_dependent_prev_vg1000_g   prop_pep  artvis0_lower_adh  pop_wide_prep_adh_effect 
pop_wide_tld_selective_hiv pep_efficacy  prob_test_pop_wide_tld_prep death_r_iris_pop_wide_tld rate_int_choice;
model d_n_death_hiv_50y_2_1 = 
pref_prep_oral_beta_s1 
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld    prob_onartvis0_0_to_1 prob_onartvis0_1_to_0
prob_prep_pop_wide_tld  inc_oral_prep_pref_pop_wide_tld  prep_dependent_prev_vg1000_g   prop_pep  artvis0_lower_adh  pop_wide_prep_adh_effect 
pop_wide_tld_selective_hiv pep_efficacy  prob_test_pop_wide_tld_prep  death_r_iris_pop_wide_tld P_hard_reach_w p_hard_reach_m  rate_int_choice
greater_disability_tox / solution ;
run;

ods html close; 


proc univariate data=  x; var d_n_death_hiv_50y_2_1 ; where artvis0_lower_adh = 0; run;
proc freq; tables d_n_death_hiv_50y_2_1 d_n_cd4_lt200_50y_2_1 ;  where artvis0_lower_adh = 0; run;

proc freq data=  x; tables pop_wide_tld_ce  ; where incidence1549_22 >= 0.5; run;

proc freq; tables incidence1549_44_1 incidence1549_44_2 ; run;


proc univariate ; var d_p_elig_hivneg_onprep_5y_2_1   d_p_onart_5y_2_1  d_prop_prep_inj_5y_2_1
...
proc univariate  data=  x; var d_p_vl1000_3y_2_1 d_n_death_hiv_50y_2_1; 
where artvis0_lower_adh=0;
run;
...
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld  prob_tld_hiv_concern  prob_onartvis0_1_to_0 prob_onartvis0_1_to_0
prob_prep_pop_wide_tld  inc_oral_prep_pref_pop_wide_tld  prep_dependent_prev_vg1000_g   prop_pep  artvis0_lower_adh  pop_wide_prep_adh_effect 
...
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld  prob_tld_hiv_concern  prob_onartvis0_1_to_0 prob_onartvis0_1_to_0
prob_prep_pop_wide_tld  inc_oral_prep_pref_pop_wide_tld  prep_dependent_prev_vg1000_g   prop_pep  artvis0_lower_adh  pop_wide_prep_adh_effect 
low_prep_inj_uptake  pop_wide_tld_selective_hiv death_r_iris_pop_wide_tld prep_dependent_prev_vg1000 adh_pattern prob_vl_meas_done
rate_int_choice incr_rate_int_low_adh pr_art_init rate_choose_stop_prep_oral rr_mort_tdf_prep rate_exp_set_lower_p_vl1000
prob_loss_at_diag
/ solution ;
where artvis0_lower_adh=0;
run;
...
proc means ; var d_n_death_hiv_50y_2_1  ; where rr_interrupt_pop_wide_tld =1/1.5; run;
proc means ; var d_n_death_hiv_50y_2_1  ; where rr_interrupt_pop_wide_tld =1/  5; run;
...
rr_return_pop_wide_tld rr_interrupt_pop_wide_tld  prob_tld_hiv_concern  prob_onartvis0_1_to_0 prob_onartvis0_1_to_0
prob_prep_pop_wide_tld  inc_oral_prep_pref_pop_wide_tld  prep_dependent_prev_vg1000_g   prop_pep  artvis0_lower_adh  pop_wide_prep_adh_effect 
low_prep_inj_uptake  pop_wide_tld_selective_hiv death_r_iris_pop_wide_tld prep_oral_efficacy
/ solution ;
run;
