...
* extra rate paramters for which values were previously hard coded below ; * change made 17_11_19; 
...
rate_lost  rate_return  rate_restart  pr_art_init pr_switch_line
...
if t ge 2 then do; 
	...
	if hivtest_type=1 then do;
		...
		if primary{t}=1 and tested{t}=1 and u lt sens_primary then do;
			...
			if prep   =1 and pop_wide_tld_prep ne 1 then do;
				prep   =0; prep_ever=.; dt_prep_s=.; dt_prep_e=.; o_3tc=0; o_ten=0; tcur{t}=.; nactive{t}=.;
			end; * need to check why these higher than 1;
			if       .    lt u lt 0.125 then risk_incr_res_primary=fold_incr_res_primary; *infection in days 0-10;
			else if 0.125 le u le 0.25  then risk_incr_res_primary=fold_incr_res_primary * (2.75 / 3); *infection in days 10-20;
			else if 0.25  le u le 0.375 then risk_incr_res_primary=fold_incr_res_primary * (2.5 / 3); *infection in days 20-30;
			else if 0.375 le u le 0.5   then risk_incr_res_primary=fold_incr_res_primary * (2.25 / 3); *infection in days 30-40;
			else if 0.5   le u le 0.625 then risk_incr_res_primary=fold_incr_res_primary * (2 / 3);  *infection in days 40-50;
			else if 0.625 le u le 0.75  then risk_incr_res_primary=fold_incr_res_primary * (1.75 / 3); *infection in days 50-60;
			else if 0.75  le u le 0.875 then risk_incr_res_primary=fold_incr_res_primary * (1.5 / 3); *infection in days 60-70;
			else if 0.875 le u lt 1     then risk_incr_res_primary=fold_incr_res_primary * (1.25/ 3); *infection in days 70-80;
		end;
		else if primary{t}=1 and tested{t}=1 and u ge sens_primary then do;
...
* dependent_on_time_step_length ;  
	...
	if hivtest_type=3 or hivtest_type=4 then risk_incr_res_primary=1*fold_incr_res_primary; 
	if hivtest_type=4 then do;
		u=uniform(0);
		...
		else if primary{t}=1 and tested{t}=1 and u ge sens_primary_ts1m then do;
			risk_incr_res_primary=1*fold_incr_res_primary; *infection in month 3/3 so do not get diagnosed;
		end;
	end;
	if hivtest_type=1 then do;
		u=uniform(0); * may be * dependent_on_time_step_length ;  
		if primary{t}=1 and tested{t}=1 and u lt sens_primary_ts1m then do;
			registd{t}=1; date1pos=caldate{t}; diagprim=caldate{t};
			if prep   =1 and pop_wide_tld_prep ne 1 then do;
				prep   =0; prep_ever=.; dt_prep_s=.; dt_prep_e=.; o_3tc=0; o_ten=0; tcur{t}=.; nactive{t}=.;
			end;
			if       .    lt u lt 0.33  then risk_incr_res_primary=3*fold_incr_res_primary; *infection in days 0-10;
			else if 0.33  le u le 0.67  then risk_incr_res_primary=2.75*fold_incr_res_primary; *infection in days 10-20;
			else if 0.67  le u          then risk_incr_res_primary=2.5*fold_incr_res_primary; *infection in days 20-30;
...
* VISIT FREQUENCY  ;
	...
	if onart{t}=1 then tcur{t}=tcur{t-1}+1; * dependent_on_time_step_length ;  
	if prep   =1 then tcur{t}=tcur{t-1}+1;  * dependent_on_time_step_length ;  
	...
	* for a person on ten-3tc prep at the time of adoption of pop wide tld prep;
	...
	if toffart{t-1} ge 0 and onart{t-1}=0 then do;
		toffart{t}=toffart{t-1}+1; * dependent_on_time_step_length ;  
		if prep    ne 1 then do;
...
if t ge 2 then do; 
	if . < c{t-1} < 0 then c{t-1}=0; csqr{t-1}=sqrt(c{t-1}); * dependent_on_time_step_length ;  
end;
if . < c{t} < 0 then c{t}=0;
csqr{t}=sqrt(c{t});
...
* if adc who3_event last longer than 1 month * dependent_on_time_step_length ;  
		...
		if (adc{t-1}=1 or (tb{t-2}=0 and tb{t-1}=1) or who3_event{t-1}=1) then do;
			unitest=uniform(0);if unitest<rate_test  then do;
				...
				* below * dependent_on_time_step_length ;
	...
	*Cost of diagnosing a person is higher 25$, than the cost of a negative result;
	if hiv{t}=1 and tested{t}=1 and registd{t-1} ne 1 and prep_falseneg ne 1 then do;	*V*hiv(t)=1 is valid for everybody;
		...
		if t ge 3 and unisens lt sens_vct then do; 
			registd{t}=1; date1pos=caldate{t}; * below may be * dependent_on_time_step_length ;
...
* Loss to follow-up while off ART;
	if t ge 2 and onart{t}=0 and lost{t-1}=0 then do;
		e=uniform(0); * may be * dependent_on_time_step_length ;
...
* Returning to clinic after loss to follow-up ;
	...
	e_rate_return = eff_rate_return; * may be * dependent_on_time_step_length ;
	if higher_newp_less_engagement = 1 and t ge 2 and newp{t-1} > 1 then e_rate_return = eff_rate_return / 1.5;

	s=uniform(0);
	if adhav >= 0.8 then do;  * dependent_on_time_step_length ;
	...
	if 0.5 <= adhav < 0.8 then do;  * dependent_on_time_step_length ;
		if t ge 2 and lost{t-1}=1 and registd{t-1}=1 and
		((adc{t-1}=1 and s < prob_return_adc) or s < e_rate_return/2) then do;return_{t}=1;lost{t}=0;visit{t}=1;end;
	end;
	if adhav < 0.5 then do; * dependent_on_time_step_length ;
		if t ge 2 and lost{t-1}=1 and registd{t-1}=1 and
...
* below may be * dependent_on_time_step_length ;
	...
	if t ge 4 and caldate{t}=time0 then do; * dependent_on_time_step_length ; 
...
* WHETHER TO INITIATE 3rd LINE HAART (FOR PEOPLE WHO FAILED second LINE AND STILL ON second LINE);
	if  third_line=1 then do;
		...
		and visit{t}=1 then do;  * dependent_on_time_step_length ; 
	...
	* restart_pi_after_dtg_fail   -  may18;
	if reg_option in (103 104 110 111 114 116 117 118 119) and f_dol=1 and o_dol=1 and o_taz ne 1 and o_lpr ne 1 and (p_taz=1 or p_lpr=1) then do;
		if t ge 2  and interrupt{t}=0 and visit{t}=1 then do;  * dependent_on_time_step_length ; 
...
* tox risk and persistence * dependent_on_time_step_length ;
...
* adherence between t-1 and t  (adh{t});  * dependent_on_time_step_length ;
...
* current tox can affect adherence;  * dependent_on_time_step_length ;
...
* poorer "adherence" (lower drug levels) if have "current" tb or adc;  * dependent_on_time_step_length ;
...
* occasional severe drop in adherence / absorption (more likely when on PI) (only way I can think to explain v fail with no mutations on PI);
	...
	* below likely needs to be * dependent_on_time_step_length ;
...
* much of below * dependent_on_time_step_length ;
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_lai_23_11_19_8am_&dataset_id";  
put   
