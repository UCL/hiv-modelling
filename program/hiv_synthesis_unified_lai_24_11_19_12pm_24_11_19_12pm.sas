...
still to work down from ANALYSIS PROCEDURES to label * dependent_on_time_step_length *
...
p_neph_stops_ten = 0 
	...
	if t ge 2 and o_zdv=1 and c_head_tm1 =0 and d < 0.1 then c_head=1;
	if o_zdv=1 and c_head_tm1=1 then c_head=1; if c_head_tm1=1 and r < 0.6 then c_head=0;
...
r_nau_start_taz_dar = 0.01
p_nau_stops_taz_dar = 0.5 
r_nau_start_zdv_lpr = 0.03
p_nau_stops_zdv_lpr = 0.5 
r_dia_start_lpr = 0.02
p_dia_stops_lpr = 0.5 
...
* dependent_on_time_step_length ;
	if t ge 2 and tcur{t} ge 2 and ((artline{t}=1 and linefail{t}=0) or (artline{t}=2 and linefail{t}=1)) and who3_event{t}=0 
...
* dependent_on_time_step_length ;		
	...
	* occurrence of who4 diseases;
	...
	if  dead{t}=0 then do;
		rate=base_rate;
		* t-1 so likely * dependent_on_time_step_length ;
...
* consider if * consider if * dependent_on_time_step_length ;
