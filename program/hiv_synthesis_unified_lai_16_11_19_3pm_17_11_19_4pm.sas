...
rate_test_startprep  rate_test_restartprep  rate_choose_stop_prep  prob_prep_restart_choice
...
* has t-1 so * dependent_on_time_step_length ;
...
if registd{t} ne 1 and caldate{t} ge (date_start_testing+3.5) and tested{t} ne 1 then do;
	...
	if t ge 2 and sw_test_6mthly=1 and sw{t}=1 and tested{t-1} ne 1 then do; * has t-1 so * has t-1 so * dependent_on_time_step_length ;
	...
	if hard_reach=0 then do;
		...
		* consider if * dependent_on_time_step_length ;
		...
		if      ever_tested ne 1  then do; * dependent_on_time_step_length ;
		...
		if ever_tested  = 1  then do;
			...
			if eff_max_freq_testing=1 then do;  * dependent_on_time_step_length ;
			...
			if eff_max_freq_testing=2 then do;*every 6 months;  * dependent_on_time_step_length ;
				if (t ge 2 and tested{t-1} ne 1) and unitest < rate_reptest then do;
					tested{t}=1;if ever_tested ne 1 then date1test=caldate{t};ever_tested=1;dt_last_test=caldate{t};
					np_lasttest=0;newp_lasttest_tested_this_per = newp_lasttest; newp_lasttest=0; 
				end;
			end;
			if eff_max_freq_testing=4 then do;*every 3 months;  * dependent_on_time_step_length ;
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_lai_16_11_19_3pm_&dataset_id";  
put   
