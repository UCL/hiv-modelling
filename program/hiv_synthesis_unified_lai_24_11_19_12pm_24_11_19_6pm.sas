...
* extra rate parameters for which values were previously hard coded below ; * change made 17_11_19; 
...
* --------------------------------------------------------------------------- ;
* running with monthly time steps - transformation of rate parameters  ;
* --------------------------------------------------------------------------- ;
...
* the parameters defined immediately above need to be in this list 
...
* note need to consider for each parameter below whether the transformation above works given the way it is coded

* note there are several repeats below

*
eprate  rate_exp_set_lower_p_vl1000  prob_pregnancy_base  rate_loss_persistence  rate_loss_nnres_pmtct_maj
initial_rate_1sttest test_rate_who4  test_rate_tb test_rate_who3  
rate_lost  rate_return  rate_restart  pr_art_init pr_switch_line  prob_supply_interrupted   prob_supply_resumed
poorer_cd4rise_fail_nn    rate_int_choice  rate_loss_acq_nnm_offart 
rate_test_onprep    prob_prep_restart  prob_prep_restart_choice  pop_wide_tld_prob_egfr
 base_rate_sw  base_rate_stop_sexwork  rate_exp_set_lower_p_vl1000
rate_testanc_inc incr_test_rate_sympt  pr_art_init  rate_lost  prob_lost_art  rate_return  rate_restart  rate_int_choice rate_loss_persistence
poorer_cd4rise_fail_nn  poorer_cd4rise_fail_ii  rate_res_ten  pr_switch_line  
rate_test_startprep  rate_test_restartprep  rate_choose_stop_prep  
cost_zdv_a  cost_3tc_a cost_ten_a cost_taf  cost_nev_a cost_efa_a cost_lpr_a  cost_taz_a cost_dol_a cost_dar_a adc_cost_a who3_cost_a tb_cost_a
cot_cost_a
vis_cost_a  cost_cascade_interventions_a prep_drug_cost cost_prep_clinic cost_prep_clinic_couns

;


* to double check abut I think following should not have transformation applied:

pr_prep_b    prob_cd4_meas_done   prob_vl_meas_done  
...
* efa;
	...
	((c_cns_tm1=1 and s < r_sw_efa_cns) or (c_ras_tm1=1 and r < r_sw_efa_ras ))
	then do;
		sw_toxicity{t}=1;
...
* nev;
		...
		((c_hep_tm1=1 and s < r_sw_nev_hep) or (c_ras_tm1=1 and r < r_sw_nev_ras ))
		then do;
			sw_toxicity{t}=1;
...
* zdv;
            ...
            ((c_nau_tm1=1 and s lt r_sw_zdv_nau) or (c_head_tm1=1 and d < r_sw_zdv_head) or (c_lip_tm1=1 and e < r_sw_zdv_lip)
            or (c_ane_tm1=1 and e < r_sw_zdv_ane) or c_lac_tm1=r_sw_zdv_lac )   then do;
                  sw_toxicity{t}=1;
...
* ten;
            s=uniform(0); if t ge 2  and interrupt{t}=0 and o_ten_tm1=1 and (c_neph_tm1=1 and s lt r_sw_ten_neph) then do;
                  sw_toxicity{t}=1;
...
* lpr;
		...
		((c_nau_tm1=1 and s < r_sw_lpr_nau ) or (c_dia_tm1=1 and r < r_sw_lpr_dia )) then do; *note nau currently does not occur with lpr;
			sw_toxicity{t}=1;
...
* taz;
		...
		((c_nau_tm1=1 and s < r_sw_taz_nau ) or (c_dia_tm1=1 and r < r_sw_taz_dia )) then do; *note nau currently does not occur with taz;
			sw_toxicity{t}=1;
...
* dol ;
	s=uniform(0);  if t ge 2  and interrupt{t}=0 and o_dol_tm1=1 and (c_cns_tm1=1 and s < r_sw_dol_cns) then do; 
		sw_toxicity{t}=1;
...
* to here ;
	...
	***Stage 2-3;
	* may be * dependent_on_time_step_length ;
...
* consider length primary may be * dependent_on_time_step_length ;
...
* consider length of primary infection * dependent_on_time_step_length ;
...
if gender=1 then do;
	...
	if sw&j = 1 		   then do;  ever_tested_sw   =ever_tested; diag_sw   =registd&j; onart_sw   =onart&j;vs_sw=vl1000&j; end;
...
* consider if pregnant will be 1 only for 1 period * dependent_on_time_step_length ;  
...
* uses tcur so * dependent_on_time_step_length ;
...
d=&j;  discount = (0.9924141173)**(d-125);
...
* due to &x consider if * dependent_on_time_step_length ;
...
* due to tm1 * dependent_on_time_step_length ;
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_lai_23_11_19_4pm_&dataset_id";  
put   
