...
* === LAI NOTES 
 ...
 1_12_19 1pm - open up ts1m code so runs monthly

 29_11_19 10am - ts1m code in place but commented out
...
first just build in the possibility of changing time step to 1 month manually but commenting out current 3 month code as needed
it may be possible to adopt this program straight away as unified code, keeping the 1 month update commented out until we are sure
that is working 

then go on to making the other coding developments needed for lai

issues to think about: 

length of primary - suggest still one period but when considering primary infection consider caldate - date1pos <= 0.25
how to define being in care if visit=1 once every 3 months ? maybe keep visit=1 for each period in care so visit variable changed to in_care ?
record date last vm (etc) instead of using t-1 tm1 etc
length of who3, adc - suggest keep as lasting 1 period
length of tb - suggest lasts 1 period and treatment lasts 6 months
.....

note if leave as is then primary infection only lasts 1 month when time step is 1 month


rate_per_mth = 1 - (1 - rate_per_3_mths)**1/3

only use this program for lai analysis until confident no errors

any {t-1} or _tm1 etc is * dependent_on_time_step_length ;
* also check all occurrences of .25 ;

* fractions of years by period
...
note that when add rilpivirine need to include this wherever efa or nev currently are, including for newmut risk when adh < 0.5

bear in mind current code in which there is a risk of nnrti resistance arising as a result of stopping art with one of these drugs included
...
* fx;		r=uniform(0);  if r < 0.1 then fx = 0.8; if 0.1 <= r < 0.2 then fx = 0.8; if 0.2 <= r < 0.4 then fx = 0.9; if 0.4 <= r < 0.6 then fx = 1.0;if 0.6 <= r < 0.8 then fx = 1.1; if 0.8 <= r then fx=1.3; 
* gx;		r=uniform(0);  if r < 0.9 then gx = 1.0; if 0.9 <= r  then gx = 1.5;  
...
* cascade of care;
		...
		if _u1 < 0.9 then cascade_care_improvements = 1;
...
* reg_option_switch_2020;	
						...
						if 0.60 <= _u49 < 0.9 then do; reg_option = 103; reg_option_switch_2020 = 103; end;
...
* pop_wide_tld_2020;	pop_wide_tld_2020 = 0;
						if _u47 < 0.20 then do;  
							...
							prob_prep_pop_wide_tld = 0.10; 
							prep_improvements = 0;  * this is instead of current type of prep program;
						end;
						* pop wide tld - although all have availability, is assumed that use is if ( prep_elig = 1 or ( ever_newp = 1 and ever_tested ne 1 ) );
...
* decr_r_choose_stop_prep_2020;
						...
						if _u30 < 0.80 then do; 
							...
							eff_rate_choose_stop_prep = eff_rate_choose_stop_prep = 0.05 ; 
...
* ts1m: note this is 1 period on prep so is 1 month if ts1m ; 
...
proc print; var fx caldate&j hiv&j infection v&x v&j vc&x vc&j c&j adc&j registd&j naive&j toffart&j onart&j nactive&j adh&j death ;
where (age&j ge 15 or death ne .) and (hiv&j = 1 or caldate_never_dot < 1989.33);
run;

/*
proc lifetest; time survaids * aidsyn(0);
where age_infection > 25;
run; 

proc lifetest; time survwho34 * who34yn(0);
where age_infection > 25;
run; 

proc lifetest; time survcl200 * cl200yn(0);
where age_infection > 25;
run; 
*/
...
* file "C:\Loveleen\Synthesis model\Multiple enhancements\multiple_enhancements_&dataset_id";  
  file "/home/rmjlaph/Scratch/_output_lai_3_12_19_8am_&dataset_id";  
put   
