...
poorer_cd4rise_fail_nn    rate_int_choice  rate_loss_acq_nnm_offart prob_cd4_meas_done prob_vl_meas_done
...
poorer_cd4rise_fail_nn  poorer_cd4rise_fail_ii  rate_res_ten  pr_switch_line  prob_vl_meas_done
...
u_vfail1_this_period=0;  * may be * dependent_on_time_step_length ;
...
o_dol_at_risk_uvfail=.;u_vfail1_dol_this_period=.; * dependent_on_time_step_length ;
...
* consider if below * dependent_on_time_step_length ;
...
* below tox risk and persistence * dependent_on_time_step_length ;
	...
	* note that increased mortality risk due to weight rise on dol is independent of weightg as a tox (potentially leading to discontinuation)
	...
	if t ge 2 and o_efa=1 then do;
		if c_cns_tm1=0 and r < 0.1 and tcur{t} < 1 then c_cns=1;
		if c_cns_tm1=1 and r < 0.8 and tcur{t} < 1 then c_cns=1;
		if c_cns_tm1=1 and tcur{t} >= 1 and r < 0.90 then c_cns=1;
	end;


	rr=uniform(0);
	if t ge 2 and o_dol=1 then do;  * take incidence as half for efa but with same persistence rate; * apr16; * changed since nih talk; 
		if c_cns_tm1=0 and rr < (0.05 * rel_dol_tox) and tcur{t} < 1 then c_cns=1;
		if c_cns_tm1=1 and rr < 0.4 and tcur{t} < 1 then c_cns=1;
		if c_cns_tm1=1 and tcur{t} >= 1 and rr < 0.90 then c_cns=1;
	end;
	...
	* no c_lip=0 because stays once have it even if stop drugs;
	...
	c_hep=0;
	r=uniform(0);
	if  o_nev=1 and tcur{t} <= 0.5 and r < 0.02 then c_hep=1;
