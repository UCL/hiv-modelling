* Note this create wide file compiles the results from the model runs on 26/2/26 where self-testing is kept in the Minimal scenario;
* Some small typos have been corrected since the 24/1/26 version;
...
proc export data=outputs_&op_num										/* export to csv in output folder */
	outfile= "C:\Users\rmjlja9\UCL Dropbox\Jennifer Smith\hiv synthesis ssa unified program\output files\hiv_control_zimbabwe\hiv_control_zim_20260226_out\outputs_&op_num..csv" 
	dbms=csv replace; 
	putnames=no;
run;
