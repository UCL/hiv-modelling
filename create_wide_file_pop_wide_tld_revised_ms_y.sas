...
* Extract from model runs: number of people with HIV on TLD as PrEP/PEP ?  
...
libname b "C:\Users\w3sth\Dropbox (UCL)\hiv synthesis ssa unified program\output files\pop_wide_tld\pop_wide_tld_revision_y_out\";
...
* re-start sas - uncomment libname statement; 
...
proc sort data= a.pwt_rev_y_b out= a.pwt_rev_y_c ; by run option; run;

proc freq data=a.pwt_rev_y_c ; tables run; where cald = 2022; run;
...
data    a.l_pwt_rev_y     ; set a.pwt_rev_y_c;  
...
if cald=2024.5 and option=1; * note this is 2 because this is from dataset y where options are still 0, 1, 2 ; 
