...
* p_hiv_mm;						p_hiv_mm = s_hiv1564mm/s_alive1564mm;
...
***This section of code is used to create calibration graphs in Excel;

****Baseline data for calibration is from
C:\Users\Loveleen\UCL Dropbox\Loveleen bansi-matharu\hiv synthesis ssa unified program\output files\mihpsa_sa\Calibration data;
...
***Align these vars with baseline calibration sheet; 

%mend var;

%var(v=n_alive_m);			%var(v=n_alive_w);    		%var(v=n_alive);		%var(v=prevalence1549m);	%var(v=prevalence1549w);  
%var(v=prevalence1549_);	%var(v=incidence1549m);		%var(v=incidence1549w);	%var(v=incidence1549_);		%var(v=p_diag_m);    	
%var(v=p_diag_w);     		%var(v=p_diag); 			%var(v=p_onart_diag_m);	%var(v=p_onart_diag_w);		%var(v=p_onart_diag); 
%var(v=p_onart_vl1000_m);  	%var(v=p_onart_vl1000_w);  	%var(v=p_onart_vl1000_);%var(v=n_onprep_m);			%var(v=n_onprep_w);
...
proc sgplot data=d; 
Title    height=1.5 justify=center "Of men proportion mobile";
xaxis label			= 'Year'		labelattrs=(size=12)  values = (1993 to &year_end by 2)	 	 valueattrs=(size=10); 
yaxis grid label	= 'Proportion'		labelattrs=(size=12)  values = (0 to 1 by 0.1) valueattrs=(size=10);

label p50_p_mm_0 = "Option 0";
label p50_p_mm_1 = "Option 1";
label p50_p_mm_2 = "Option 2";
...
series  x=cald y=p50_p_mm_1/	lineattrs = (color=red thickness = 2);
band    x=cald lower=p5_p_mm_1 	upper=p95_p_mm_1  / transparency=0.9 fillattrs = (color=red) legendlabel= "Model 90% range";
series  x=cald y=p50_p_mm_2/	lineattrs = (color=red thickness = 2);
band    x=cald lower=p5_p_mm_2 	upper=p95_p_mm_2  / transparency=0.9 fillattrs = (color=red) legendlabel= "Model 90% range";
...
series  x=cald y=p50_p_hiv_m_0/	lineattrs = (color=red thickness = 2);
band    x=cald lower=p5_p_hiv_m_0 	upper=p95_p_hiv_m_0  / transparency=0.9 fillattrs = (color=red) legendlabel= "Model 90% range";
run;quit;
...
label p50_p_prep_any_ever_mm_0 = "Option 0";
label p50_p_prep_any_ever_mm_1 = "Option 1";
label p50_p_prep_any_ever_mm_2 = "Option 2";
