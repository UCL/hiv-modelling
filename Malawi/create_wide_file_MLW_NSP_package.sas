...
if cald=2030 and option=4 and p_onart<0.93 then b=1;
...
* Not assessing FSW so include the same cost for all scenarios;;
cost_fsw_services = s_sw_program_visit * cost_FSW_services_pppy * sf;
dcost_fsw_services = cost_fsw_services * discount;

cost_adh_support = 0; 
if option in (3,4) then cost_adh_support = s_diag * cost_AdhSupp_pppy * sf;* Assumes the cost is applied to everyone diagnosed;	
...
data a.long_gen_mlw_NSP_package;
set y;
run;
...
data a.wide_gen_mlw_NSP_package;
set wide_outputs  ;  
by run;run; 
