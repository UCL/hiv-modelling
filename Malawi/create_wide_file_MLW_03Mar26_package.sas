...
if option in (1,2,3,4,5,6,99) then cost_fsw_services = s_sw_program_visit * cost_FSW_services_pppy * sf;
dcost_fsw_services = cost_fsw_services * discount;

cost_adh_support = 0; 
if option =3 then cost_adh_support = s_diag * cost_AdhSupp_pppy * sf;* Assumes the cost is applied to everyone diagnosed;	
...
data a.long_gen_mlw_03_03_2026_package;
set y;
run;
...
data a.wide_gen_mlw_03_03_2026_package;
set wide_outputs  ;  
by run;run; 
