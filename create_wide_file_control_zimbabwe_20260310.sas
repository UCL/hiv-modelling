...
libname a "C:\Users\rmjlja9\UCL Dropbox\Jennifer Smith\hiv synthesis ssa unified program\output files\hiv_control_zimbabwe\hiv_control_zim_20260310_out\";
...
* MSM services cost applies to option 7 onwards;
cost_fsw_services = s_sw_program_visit * &cost_FSW_services_pppy * sf;		* removed  * 4 ;
cost_msm_services = 0; if option ge 7 then cost_msm_services = s_alive1549_msm / 2 * &cost_MSM_services_pppy * sf;			* removed  * 4 ;	
...
* Adherence support;
* Cost applies to option 5 onwards;
cost_adh_support = 0; if option ge 5 then cost_adh_support = s_diag * &cost_AdhSupp_pppy * sf;		* Assumes the cost is applied to everyone diagnosed (changed from on ART Feb 26);	* removed  * 4 ;
...
proc export data=outputs_&op_num										/* export to csv in output folder */
	outfile= "C:\Users\rmjlja9\UCL Dropbox\Jennifer Smith\hiv synthesis ssa unified program\output files\hiv_control_zimbabwe\hiv_control_zim_20260310_out\outputs_&op_num..csv" 
	dbms=csv replace; 
	putnames=no;
run;
